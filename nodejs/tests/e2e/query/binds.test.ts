...
describe('Query Binds', () => {
  async function insertRowsAndSelect(connection: Connection, binds: Binds): Promise<unknown[]> {
    const tableName = await createTemporaryTable(connection, 'ID NUMBER, VAL STRING');
  ...
  describe('arrayBindingThreshold connection parameter', () => {
    it('should upload binds to the SYSTEM$BIND stage above the threshold', async () => {
      const connection = await createLiveConnection({ arrayBindingThreshold: LOW_THRESHOLD });
    ...
    it('should send binds inline below the threshold', async () => {
      const connection = await createLiveConnection({ arrayBindingThreshold: HIGH_THRESHOLD });
      const selected = await insertRowsAndSelect(connection, [
        [1, 'a'],
        [2, 'b'],
        [3, 'c'],
    ...
    it('should produce identical rows via the stage and inline paths', async () => {
      ...
      const connection1 = await createLiveConnection({ arrayBindingThreshold: LOW_THRESHOLD });
      viaStage = await insertRowsAndSelect(connection1, binds);
      let viaInline: unknown[] = [];
      const connection2 = await createLiveConnection({ arrayBindingThreshold: HIGH_THRESHOLD });
      viaInline = await insertRowsAndSelect(connection2, binds);
