 ...
 *
 * What this is:
 *   A snapshot of the public type declarations from the old driver
 *   https://github.com/snowflakedb/snowflake-connector-nodejs
 *   version 3.1.0, pinned at commit 81080506bf5f4ab753503b818084d473dad40b62.
 *
 * Why it lives here:
 *   - Our tests install the old driver under an aliased package name so it can
 *     coexist with the new one. See nodejs/package.json:
 *         "snowflake-sdk-old": "npm:snowflake-sdk@^..."
 *   - The old driver declares the types as `declare module 'snowflake-sdk'`, but
 *     under the alias the package is imported as 'snowflake-sdk-old'. So this
 *     snapshot is patched: `declare module 'snowflake-sdk'` is rewritten to
 *     `declare module 'snowflake-sdk-old'` so TypeScript resolves the types for
 *     the aliased import.
 *
 * How to update (the only correct way to change this file):
 *   1. Copy the fresh index.d.ts (and referenced type files) from the old
 *      driver repo at the commit you want to pin.
 *   2. Re-apply the patch: rename 'snowflake-sdk' to 'snowflake-sdk-old' in the
 *      `declare module` statements.
 *   3. Update the commit hash and the version in the comment above.
 */
import { WIP_ConnectionOptions } from 'snowflake-sdk-old/dist/lib/connection/types';
import ErrorCodeEnum from 'snowflake-sdk-old/dist/lib/error_code';
import { GlobalConfigOptionsTyped } from 'snowflake-sdk-old/dist/lib/global_config_typed';

declare module 'snowflake-sdk-old' {
