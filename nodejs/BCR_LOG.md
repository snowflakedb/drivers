...
- Client DATE formatting (`fetchAsString`, `toJSON`, `getFormat`) ignores statement-level `DATE_OUTPUT_FORMAT`; `ALTER SESSION` is honored on the old driver (the new driver still hardcodes `YYYY-MM-DD`)
...
- Client TIME formatting ignores statement-level `TIME_OUTPUT_FORMAT`; `ALTER SESSION` is honored on both drivers
