...
CursorShape captureShape(StatementHandleWrapper& stmt) {
  CursorShape shape{};
  SQLRETURN ret = SQLNumResultCols(stmt.getHandle(), &shape.num_cols);
  REQUIRE_ODBC(ret, stmt);
  ret = SQLRowCount(stmt.getHandle(), &shape.row_count);
  REQUIRE_ODBC(ret, stmt);
  return shape;
}
}  // namespace
...
TEST_CASE("should report correct cursor shape for each result set in a DDL + DML + DDL batch",
  ...
  CursorShape create_shape = captureShape(stmt);
  ...
  REQUIRE_ODBC(ret, stmt);
  CursorShape insert_shape = captureShape(stmt);
  ...
  ret = SQLMoreResults(stmt.getHandle());
  REQUIRE_ODBC(ret, stmt);
  CursorShape drop_shape = captureShape(stmt);
...
TEST_CASE("should not open a cursor for any statement in a TCL-only batch", "[query][multistatement]") {
  ...
  for (int stmt_idx = 0; stmt_idx < 3; ++stmt_idx) {
    ...
    CursorShape shape = captureShape(stmt);
