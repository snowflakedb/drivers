// Tests that binding temporal and GUID C types to approximate SQL types
// (SQL_DOUBLE, SQL_REAL) returns an error, as these conversions are
...
TEST_CASE("should reject SQL_C_GUID bound to SQL_DOUBLE", "[c_temporal][incompatible][sql_real]") {
  ...
  // Then the driver rejects the incompatible conversion with an error
  check_incompatible_bindparam(stmt, SQL_C_GUID, SQL_DOUBLE, &guid, sizeof(guid), &ind);
}
