...
TEST_CASE("Replay: exec_direct_getdata_14col_26rows", "[dtm]") {
  ...
  // SQLExecDirect
  {
    SQLRETURN ret = SQLExecDirect(
        stmt0, sqlchar("SHOW TABLES LIKE 'iceberg\\\\_types\\\\_tbl' IN SCHEMA \"DTMREPLAYTESTDB\".\"PUBLIC\""),
        SQL_NTS);
  ...
  // SQLDescribeCol col 1
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 1, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "created_on");
    CHECK(dataType == 93);
    CHECK(colSize == 23);
    CHECK(scale == 3);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 3
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 3, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "database_name");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 4
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 4, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "schema_name");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 7
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 7, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "cluster_by");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 9
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 9, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "bytes");
    CHECK(dataType == 3);
    CHECK(colSize == 38);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 10
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 10, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "owner");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 11
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 11, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "retention_time");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // Adapted: sfengineering accounts include automatic_clustering (absent on sfctest0)
  //          and lack search_optimization* (Enterprise-only feature not enabled here).
  // SQLDescribeCol col 12
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 12, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "automatic_clustering");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 13
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 13, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "change_tracking");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 23
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 23, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "row_timestamp");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 24
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 24, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "error_logging");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLGetData col 1
  {
    ...
    SQLRETURN ret = SQLGetData(stmt0, 1, SQL_TYPE_TIMESTAMP, buf.data(), 4096, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(ind == 16);
  }
  ...
  // SQLGetData col 2
  {
    ...
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(buf.data()) == "iceberg_types_tbl");
    CHECK(ind == 17);
  }
  ...
  // SQLGetData col 3
  {
    ...
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    // Adapted: original trace had customer DB name
    CHECK(std::string(buf.data()) == "DTMREPLAYTESTDB");
    CHECK(ind == 15);
  }
  ...
  // SQLGetData col 4
  {
    ...
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(buf.data()) == "PUBLIC");
    CHECK(ind == 6);
  }
  ...
  // SQLGetData col 5
  {
    ...
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(buf.data()) == "TABLE");
    CHECK(ind == 5);
  }
  ...
  // SQLGetData col 10
  {
    ...
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    // Adapted: owner role is environment-specific
    CHECK(ind > 0);
  }
  ...
  // SQLGetData col 11
  {
    ...
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(buf.data()) == "1");
    CHECK(ind == 1);
  }

  // SQLGetData col 12 — automatic_clustering
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
    SQLRETURN ret = SQLGetData(stmt0, 12, SQL_CHAR, buf.data(), 4096, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    // Adapted: automatic_clustering default varies by table creation
    CHECK(ind > 0);
  }

  // SQLGetData col 13 — change_tracking
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
    SQLRETURN ret = SQLGetData(stmt0, 13, SQL_CHAR, buf.data(), 4096, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    // Adapted: change_tracking default varies by table creation
    CHECK(ind > 0);
  }

  // SQLGetData col 14 — is_external
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
  ...
  // SQLGetData col 15 — enable_schema_evolution
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
  ...
  // SQLGetData col 16 — owner_role_type
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
    SQLRETURN ret = SQLGetData(stmt0, 16, SQL_CHAR, buf.data(), 4096, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(buf.data()) == "ROLE");
    CHECK(ind == 4);
  }

  // SQLGetData col 17 — is_event
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
    SQLRETURN ret = SQLGetData(stmt0, 17, SQL_CHAR, buf.data(), 4096, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(buf.data()) == "N");
    CHECK(ind == 1);
  }

  // SQLGetData col 18 — is_hybrid
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
  ...
  // SQLGetData col 19 — is_iceberg
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
    SQLRETURN ret = SQLGetData(stmt0, 19, SQL_CHAR, buf.data(), 4096, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    // Adapted: is_iceberg depends on how table was created
    CHECK(ind == 1);
  }

  // SQLGetData col 20 — is_dynamic
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
    SQLRETURN ret = SQLGetData(stmt0, 20, SQL_CHAR, buf.data(), 4096, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(buf.data()) == "N");
    CHECK(ind == 1);
  }

  // SQLGetData col 21 — is_immutable
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
  ...
  // SQLGetData col 22 — is_interactive
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
  ...
  // SQLGetData col 23 — row_timestamp
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
  ...
  // SQLGetData col 24 — error_logging
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
    SQLRETURN ret = SQLGetData(stmt0, 24, SQL_CHAR, buf.data(), 4096, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    // Adapted: error_logging is a new column not present in the original trace;
    // exact value not asserted but must not be SQL_NULL_DATA or SQL_NO_TOTAL.
    CHECK(ind >= 0);
  }
