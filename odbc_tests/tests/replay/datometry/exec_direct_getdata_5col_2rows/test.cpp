...
TEST_CASE("Replay: exec_direct_getdata_5col_2rows", "[dtm]") {
  ...
  // SQLExecDirect
  {
    SQLRETURN ret = SQLExecDirect(
        stmt0, sqlchar("show terse objects like 'dtm386\\\\_ambig\\\\_tbl' in \"DTMREPLAYTESTDB\".\"PUBLIC\""),
        SQL_NTS);
  ...
  // SQLGetData col 2
  {
    ...
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    // Adapted: table created with quoted lowercase identifier
    CHECK(std::string(buf.data()) == "dtm386_ambig_tbl");
    CHECK(ind == 16);
  }
  ...
  // Adapted: original trace had 2 matching objects; test DB has 1
  // SQLFetch — no more rows
  {
    SQLRETURN ret = SQLFetch(stmt0);
