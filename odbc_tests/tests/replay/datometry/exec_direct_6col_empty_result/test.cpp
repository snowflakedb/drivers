...
TEST_CASE("Replay: exec_direct_6col_empty_result", "[dtm]") {
  ...
  // SQLExecDirect
  {
    SQLRETURN ret = SQLExecDirect(
        stmt0, sqlchar("select * from \"DTMREPLAYTESTDB\".\"__DTM_MDSTORE\".\"__DTM_MDSTORE_TABLE\" where 1=0"),
        SQL_NTS);
  ...
  // SQLDescribeCol col 3
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 3, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "COLNAME");
    CHECK(dataType == 12);
    CHECK(colSize == 256);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 4
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 4, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "PROPNAME");
    CHECK(dataType == 12);
    CHECK(colSize == 256);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 5
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 5, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "PROPVALUE");
    CHECK(dataType == 12);
  ...
  // SQLDescribeCol col 6
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 6, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "SEQNO");
    CHECK(dataType == 3);
    CHECK(colSize == 38);
    CHECK(scale == 0);
    CHECK(nullable == 1);
