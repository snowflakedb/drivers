...
TEST_CASE("Replay: exec_direct_bindcol_6col_1rows", "[dtm]") {
  ...
  // SQLExecDirect
  {
    SQLRETURN ret = SQLExecDirect(stmt0, sqlchar("show schemas like 'public'"), SQL_NTS);
  ...
  // Trace captured DSI driver-specific multi-result-set transitions for SHOW
  // SCHEMAS that differ from unixODBC driver manager behavior. Adjusted to
  // match standard behavior: after last row, Fetch returns NO_DATA and
  // MoreResults returns NO_DATA (single result set).

  // Drain remaining rows (default DSN DB may have multiple matching schemas)
  {
    SQLRETURN ret;
    do {
      ret = SQLFetch(stmt0);
  ...
  // SQLMoreResults — no more result sets
  {
