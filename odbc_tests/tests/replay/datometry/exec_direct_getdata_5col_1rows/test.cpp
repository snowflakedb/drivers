...
TEST_CASE("Replay: exec_direct_getdata_5col_1rows", "[dtm]") {
  ...
  // SQLExecDirect
  {
    SQLRETURN ret = SQLExecDirect(
        stmt0, sqlchar("show terse objects like 'hq18673\\\\_t' in \"DTMREPLAYTESTDB\".\"PUBLIC\""), SQL_NTS);
  ...
  // SQLGetData col 2
  {
    ...
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(buf.data()) == "hq18673_t");
    CHECK(ind == 9);
  }
