...
TEST_CASE("Replay: exec_direct_getdata_1col_1rows", "[dtm]") {
  ...
  // SQLExecDirect
  {
    SQLRETURN ret =
        SQLExecDirect(stmt0,
                      sqlchar("select count(*) from \"DTMREPLAYTESTDB\".\"__DTM_MDSTORE\".\"__DTM_MDSTORE_TABLE\" "
                              "where lower(schemaname)  =  'public' and lower(objectname)  =  'x4'"),
                      SQL_NTS);
  ...
  // SQLDescribeCol col 1
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 1, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "COUNT(*)");
    CHECK(dataType == 3);
    CHECK(colSize == 18);
    CHECK(scale == 0);
    CHECK(nullable == 0);
