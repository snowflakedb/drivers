...
TEST_CASE("Replay: exec_direct_getdata_13col_5rows", "[dtm]") {
  ...
  {
    SQLRETURN ret =
        SQLExecDirect(stmt0, sqlchar("desc table \"DTMREPLAYTESTDB\".\"PUBLIC\".\"hq17907_accum\""), SQL_NTS);
  ...
  // SQLGetData col 2
  {
    ...
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(buf.data()) == "NUMBER(7,4)");
    CHECK(ind == 11);
  }
  ...
  // SQLGetData col 2
  {
    ...
    SQLRETURN ret = SQLGetData(stmt0, 2, SQL_CHAR, buf.data(), 4096, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(buf.data()) == "NUMBER(5,3)");
    CHECK(ind == 11);
  }
  ...
  // SQLGetData col 1
  {
    ...
    SQLRETURN ret = SQLGetData(stmt0, 1, SQL_CHAR, buf.data(), 4096, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(buf.data()) == "depth");
    CHECK(ind == 5);
  }
