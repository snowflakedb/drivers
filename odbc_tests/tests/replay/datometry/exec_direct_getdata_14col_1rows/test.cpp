...
TEST_CASE("Replay: exec_direct_getdata_14col_1rows", "[dtm]") {
  ...
  {
    SQLRETURN ret =
        SQLExecDirect(stmt0, sqlchar("show schemas like 'public' in database \"DTMREPLAYTESTDB\""), SQL_NTS);
  ...
  // SQLDescribeCol col 3
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 3, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "is_default");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 4
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 4, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "is_current");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 8
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 8, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "options");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 11
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 11, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "classification_profile_database");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 12
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 12, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "classification_profile_schema");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLDescribeCol col 13
  {
    ...
    SQLRETURN ret = SQLDescribeCol(stmt0, 13, reinterpret_cast<SQLCHAR*>(colName), 255, nullptr, &dataType, &colSize,
                                   &scale, &nullable);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(colName) == "classification_profile");
    CHECK(dataType == 12);
    CHECK(colSize == 16777216);
    CHECK(scale == 0);
    CHECK(nullable == 1);
  ...
  // SQLGetData col 3
  {
    ...
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    // Adapted: in DTMREPLAYTESTDB, PUBLIC is the default schema
    CHECK(ind == 1);
  }
  ...
  // SQLGetData col 5 — database name
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
