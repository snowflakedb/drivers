...
TEST_CASE("Replay: exec_direct_getdata_23col_1rows", "[dtm]") {
  ...
  // SQLExecDirect
  {
    SQLRETURN ret = SQLExecDirect(
        stmt0, sqlchar("SHOW TABLES LIKE 'ren\\\\_test\\\\_t\\\\_1\\\\_1' IN SCHEMA \"DTMREPLAYTESTDB\".\"PUBLIC\""),
        SQL_NTS);
  ...
  // SQLGetData col 2
  {
    ...
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt0), OdbcMatchers::IsSuccess());
    CHECK(std::string(buf.data()) == "ren_test_t_1_1");
    CHECK(ind == 14);
  }
  ...
  // SQLGetData col 24 — error_logging (new column; value not asserted)
  {
    std::vector<char> buf(4097, 0);
    SQLLEN ind = 0;
