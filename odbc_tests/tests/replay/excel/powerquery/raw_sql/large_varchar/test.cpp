...
TEST_CASE("Replay: excel powerquery raw_sql large_varchar", "[excel][powerquery][raw_sql][flaky]") {
  ...
  // SQLExecDirect
  {
    SQLRETURN ret = SQLExecDirect(
        stmt0, sqlchar("SELECT LARGEVARCHAR FROM ODBCMETADATATESTDB.DATATYPETESTS.LARGELOBS WHERE ROWKIND = 'FULL';"),
        SQL_NTS);
  ...
  {
    SQLRETURN ret = SQLExecDirect(
        stmt1, sqlchar("SELECT LARGEVARCHAR FROM ODBCMETADATATESTDB.DATATYPETESTS.LARGELOBS WHERE ROWKIND = 'FULL';"),
        SQL_NTS);
  ...
  // SQLGetData col 1
  {
    ...
    // Old driver returns SQL_NO_TOTAL for chunked SQL_C_WCHAR (BD#23).
    // New driver returns the remaining byte count in the DM encoding.
    OLD_DRIVER_ONLY("BD#23") { CHECK(ind == SQL_NO_TOTAL); }
    NEW_DRIVER_ONLY("BD#23") {
      NON_IODBC { CHECK(ind == 262144); }
      // iODBC uses 4-byte UTF-32; ind = remaining_chars * 4 (BD#23 + BD#79).
      IODBC_ONLY { CHECK(ind == 524288); }
    }
  }
  ...
  {
    SQLLEN ind = 0;
    std::vector<char> buf(2050, static_cast<char>(0xFF));
    SQLRETURN ret = SQLGetData(stmt1, 1, SQL_C_WCHAR, buf.data(), 2050, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt1), OdbcMatchers::IsSuccessWithInfo());
    // Old driver returns SQL_NO_TOTAL for chunked SQL_C_WCHAR (BD#23).
    // New driver returns the remaining byte count in the DM encoding.
    OLD_DRIVER_ONLY("BD#23") { CHECK(ind == SQL_NO_TOTAL); }
    NEW_DRIVER_ONLY("BD#23") {
      NON_IODBC { CHECK(ind == 260098); }
      // iODBC uses 4-byte UTF-32; ind = remaining_chars * 4 (BD#23 + BD#79).
      IODBC_ONLY { CHECK(ind == 522244); }
    }
  }
  ...
  {
    SQLLEN ind = 0;
    std::vector<char> buf(4098, static_cast<char>(0xFF));
    SQLRETURN ret = SQLGetData(stmt1, 1, SQL_C_WCHAR, buf.data(), 4098, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt1), OdbcMatchers::IsSuccessWithInfo());
    // Old driver returns SQL_NO_TOTAL for chunked SQL_C_WCHAR (BD#23).
    // New driver returns the remaining byte count in the DM encoding.
    OLD_DRIVER_ONLY("BD#23") { CHECK(ind == SQL_NO_TOTAL); }
    NEW_DRIVER_ONLY("BD#23") {
      NON_IODBC { CHECK(ind == 258050); }
      // iODBC uses 4-byte UTF-32; ind = remaining_chars * 4 (BD#23 + BD#79).
      IODBC_ONLY { CHECK(ind == 520200); }
    }
  }
  ...
  {
    SQLLEN ind = 0;
    std::vector<char> buf(8194, static_cast<char>(0xFF));
    SQLRETURN ret = SQLGetData(stmt1, 1, SQL_C_WCHAR, buf.data(), 8194, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt1), OdbcMatchers::IsSuccessWithInfo());
    // Old driver returns SQL_NO_TOTAL for chunked SQL_C_WCHAR (BD#23).
    // New driver returns the remaining byte count in the DM encoding.
    OLD_DRIVER_ONLY("BD#23") { CHECK(ind == SQL_NO_TOTAL); }
    NEW_DRIVER_ONLY("BD#23") {
      NON_IODBC { CHECK(ind == 253954); }
      // iODBC uses 4-byte UTF-32; ind = remaining_chars * 4 (BD#23 + BD#79).
      IODBC_ONLY { CHECK(ind == 516108); }
    }
  }
  ...
  {
    SQLLEN ind = 0;
    std::vector<char> buf(16386, static_cast<char>(0xFF));
    SQLRETURN ret = SQLGetData(stmt1, 1, SQL_C_WCHAR, buf.data(), 16386, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt1), OdbcMatchers::IsSuccessWithInfo());
    // Old driver returns SQL_NO_TOTAL for chunked SQL_C_WCHAR (BD#23).
    // New driver returns the remaining byte count in the DM encoding.
    OLD_DRIVER_ONLY("BD#23") { CHECK(ind == SQL_NO_TOTAL); }
    NEW_DRIVER_ONLY("BD#23") {
      NON_IODBC { CHECK(ind == 245762); }
      // iODBC uses 4-byte UTF-32; ind = remaining_chars * 4 (BD#23 + BD#79).
      IODBC_ONLY { CHECK(ind == 507920); }
    }
  }
  ...
  {
    SQLLEN ind = 0;
    std::vector<char> buf(32770, static_cast<char>(0xFF));
    SQLRETURN ret = SQLGetData(stmt1, 1, SQL_C_WCHAR, buf.data(), 32770, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt1), OdbcMatchers::IsSuccessWithInfo());
    // Old driver returns SQL_NO_TOTAL for chunked SQL_C_WCHAR (BD#23).
    // New driver returns the remaining byte count in the DM encoding.
    OLD_DRIVER_ONLY("BD#23") { CHECK(ind == SQL_NO_TOTAL); }
    NEW_DRIVER_ONLY("BD#23") {
      NON_IODBC { CHECK(ind == 229378); }
      // iODBC uses 4-byte UTF-32; ind = remaining_chars * 4 (BD#23 + BD#79).
      IODBC_ONLY { CHECK(ind == 491540); }
    }
  }
  ...
  {
    SQLLEN ind = 0;
    std::vector<char> buf(65538, static_cast<char>(0xFF));
    SQLRETURN ret = SQLGetData(stmt1, 1, SQL_C_WCHAR, buf.data(), 65538, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt1), OdbcMatchers::IsSuccessWithInfo());
    // Old driver returns SQL_NO_TOTAL for chunked SQL_C_WCHAR (BD#23).
    // New driver returns the remaining byte count in the DM encoding.
    OLD_DRIVER_ONLY("BD#23") { CHECK(ind == SQL_NO_TOTAL); }
    NEW_DRIVER_ONLY("BD#23") {
      NON_IODBC { CHECK(ind == 196610); }
      // iODBC uses 4-byte UTF-32; ind = remaining_chars * 4 (BD#23 + BD#79).
      IODBC_ONLY { CHECK(ind == 458776); }
    }
  }
  ...
  {
    SQLLEN ind = 0;
    std::vector<char> buf(131074, static_cast<char>(0xFF));
    SQLRETURN ret = SQLGetData(stmt1, 1, SQL_C_WCHAR, buf.data(), 131074, &ind);
    CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt1), OdbcMatchers::IsSuccessWithInfo());
    OLD_DRIVER_ONLY("BD#23") { CHECK(ind == SQL_NO_TOTAL); }
    NEW_DRIVER_ONLY("BD#23") {
      NON_IODBC { CHECK(ind == 131074); }
      // iODBC uses 4-byte UTF-32; ind = remaining_chars * 4 (BD#23 + BD#79).
      IODBC_ONLY { CHECK(ind == 393244); }
    }
  }
  ...
  {
    SQLLEN ind = 0;
    std::vector<char> buf(262146, static_cast<char>(0xFF));
    SQLRETURN ret = SQLGetData(stmt1, 1, SQL_C_WCHAR, buf.data(), 262146, &ind);
    // SQLWCHAR is 4 bytes (UTF-32) under iODBC and 2 bytes (UTF-16) otherwise.
    // Under 2-byte DMs the preceding chunked reads have drained the column, so
    // this read returns the final "X" untruncated. Under iODBC each prior chunk
    // consumed half as many code units, so 65,544 chars remain — more than fit
    // in this 262,146-byte buffer (capacity: 65,535 code units) — so it still
    // truncates (BD#23 + BD#79). ind = 65,544 * 4 = 262,176.
    NON_IODBC {
      CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt1), OdbcMatchers::IsSuccess());
    ...
    IODBC_ONLY {
      CHECK_THAT(OdbcResult(ret, SQL_HANDLE_STMT, stmt1), OdbcMatchers::IsSuccessWithInfo());
      // Truncated: the buffer is filled with (capacity - 1) 'X' code units plus a
      // null terminator, while ind reports the full remaining byte count.
