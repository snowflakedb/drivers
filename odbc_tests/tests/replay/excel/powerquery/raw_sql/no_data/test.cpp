...
TEST_CASE("Replay: excel powerquery raw_sql no_data", "[excel][powerquery][raw_sql]") {
  ...
  // SQLExecDirect
  {
    SQLRETURN ret = SQLExecDirect(
        stmt0, sqlchar("SELECT * FROM ODBCMETADATATESTDB.DATATYPETESTS.ALLDATATYPES WHERE 1=0;"), SQL_NTS);
  ...
  {
    SQLRETURN ret = SQLExecDirect(
        stmt1, sqlchar("SELECT * FROM ODBCMETADATATESTDB.DATATYPETESTS.ALLDATATYPES WHERE 1=0;"), SQL_NTS);
