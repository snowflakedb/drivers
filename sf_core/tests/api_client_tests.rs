...
fn test_put_overwrite_true() {
    ...
    let result = client.execute_query(&select_sql);
    let mut arrow_helper = ArrowResultHelper::from_result(result);
    arrow_helper.assert_equals_single_row(vec!["updated".to_string(), "data".to_string(), "2".to_string()]);
}
...
fn test_put_overwrite_false() {
    ...
    let result = client.execute_query(&select_sql);
    let mut arrow_helper = ArrowResultHelper::from_result(result);
    arrow_helper.assert_equals_single_row(vec!["original".to_string(), "data".to_string(), "1".to_string()]);
}
...
fn test_put_overwrite_false_multiple_files_mixed_status() {
    ...
    assert_eq!(sorted_results[0].status, "UPLOADED", "file1 should be uploaded (new file)");
    
    assert_eq!(sorted_results[1].source, file2_name);
    assert_eq!(sorted_results[1].status, "SKIPPED", "file2 should be skipped (already exists)");
    
    assert_eq!(sorted_results[2].source, file3_name);
    assert_eq!(sorted_results[2].status, "UPLOADED", "file3 should be uploaded (new file)");
    ...
    assert_eq!(data_vector[0], vec!["file1".to_string(), "content".to_string(), "1".to_string()]);
    assert_eq!(data_vector[1], vec!["file2".to_string(), "content".to_string(), "2".to_string()]);
    assert_eq!(data_vector[2], vec!["file3".to_string(), "content".to_string(), "3".to_string()]);
}