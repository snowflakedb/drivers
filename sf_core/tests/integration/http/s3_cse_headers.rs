...
//! returns raw bytes when those headers are absent or the wrap is not this
//! query-stage master key (git-stage objects), and fails GET when a CSE-sized
//! wrap cannot unwrap.
...
use sf_core::file_manager::{FileManagerError, MultipartParams, TransferCtx, download_single_file};
...
async fn try_download_from_mock(
    server: &MockServer,
    encryption_material: Option<EncryptionMaterial>,
) -> Result<tempfile::TempDir, FileManagerError> {
...
async fn download_from_mock(
    ...
    try_download_from_mock(server, encryption_material)
        .await
...
fn git_stage_wrong_length_wrap_headers(material: &EncryptionMaterial) -> Vec<(String, String)> {
    use base64::Engine;
...
fn git_stage_placeholder_headers() -> Vec<(String, String)> {
    use base64::Engine;
    use base64::engine::general_purpose::STANDARD;
    let mat_desc = serde_json::json!({
    ...
    vec![
        (
            "x-amz-meta-x-amz-key".to_string(),
            STANDARD.encode(b"test-key"),
        ),
        (
            "x-amz-meta-x-amz-iv".to_string(),
            STANDARD.encode(b"test-iv"),
        ),
        ("x-amz-meta-x-amz-matdesc".to_string(), mat_desc.to_string()),
    ]
}
...
async fn s3_download_returns_raw_bytes_when_cse_headers_are_git_stage_wrap() {
    ...
    mount_object(
        &server,
        raw_bytes.clone(),
        git_stage_wrong_length_wrap_headers(&material),
    )
    .await;
...
#[tokio::test(flavor = "multi_thread")]
async fn s3_download_returns_raw_bytes_when_cse_headers_are_placeholder_key() {
    let raw_bytes = b"git blob stored as plaintext\n".to_vec();
    let material = test_material();

    let server = MockServer::start().await;
    mount_object(&server, raw_bytes.clone(), git_stage_placeholder_headers()).await;
...
#[tokio::test(flavor = "multi_thread")]
async fn s3_download_returns_raw_bytes_when_placeholder_wrap_and_digest_present() {
    let raw_bytes = b"git blob stored as plaintext\n".to_vec();
    let material = test_material();
    let mut headers = git_stage_placeholder_headers();
    headers.push((
        "x-amz-meta-sfc-digest".to_string(),
        "not-a-real-digest".to_string(),
    ));

    let server = MockServer::start().await;
    mount_object(&server, raw_bytes.clone(), headers).await;
...
#[tokio::test(flavor = "multi_thread")]
async fn s3_download_fails_when_cse_shaped_wrap_is_corrupt_and_digest_present() {
    use base64::Engine;
    use base64::engine::general_purpose::STANDARD;
    ...
    let material = test_material();
    let (ciphertext, encrypted_key, iv, mat_desc_json) = encrypt(&plaintext, &material);
    let mut wrapped = STANDARD.decode(&encrypted_key).expect("decode wrap");
    let last = wrapped.len() - 1;
    wrapped[last] ^= 0xff;
    let corrupted_key = STANDARD.encode(wrapped);
    ...
    mount_object(
        ...
        ciphertext,
        vec![
            ("x-amz-meta-x-amz-key".to_string(), corrupted_key),
            ("x-amz-meta-x-amz-iv".to_string(), iv),
            ("x-amz-meta-x-amz-matdesc".to_string(), mat_desc_json),
            (
                "x-amz-meta-sfc-digest".to_string(),
    ...
    let result = try_download_from_mock(&server, Some(material)).await;
    assert!(matches!(result, Err(FileManagerError::Decryption { .. })));
}
