...
#[test]
fn should_surface_snowflake_rejection_when_authenticator_url_not_accepted_by_account_config() {
    // Given Wiremock is running with Snowflake rejecting the configured authenticator
    let fixture = OktaTestFixture::new();
    let rejection = "The specified authenticator is not accepted by your Snowflake account configuration. \
         Please contact your local system administrator to get the correct URL to use.";
    fixture
        .mock
        .mount(okta::authenticator_request_rejected(rejection));

    // When Trying to Connect
    let error = fixture
        .connect()
        .expect_err("Expected authenticator-request rejection to fail login");

    // Then Connection fails with Snowflake's authenticator-request rejection
    assert!(
        error.contains(rejection),
        "Expected Snowflake rejection in the error, got: {error}"
    );
    assert!(
        !error.contains("HTTP 400"),
        "logical failure must not be rewritten as HTTP 400, got: {error}"
    );
}

#[test]
fn should_surface_snowflake_rejection_when_authenticator_value_not_supported() {
