...
//! `AwsSdkReqwestClient::with_default_tls` instead, which applies the default
//! TLS policy and env-var proxy detection but has no connection-specific CRL,
...
impl AwsSdkReqwestClient {
    ...
    /// Builds an SDK-backing client with the driver's default TLS policy for
    /// contexts that have no connection: the
