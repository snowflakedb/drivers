...
pub async fn upload_to_azure_or_skip(
    ...
    upload_to_azure(&client, &url, sas_token.reveal(), encryption_result).await?;
    Ok(UploadStatus::Uploaded)
}

/// Downloads a file from Azure Blob Storage and returns encrypted data with metadata.
pub async fn download_from_azure(
    stage_info: &StageInfo,
    filename: &str,
) -> Result<(Vec<u8>, EncryptedFileMetadata), AzureDownloadError> {
    let client = create_azure_client()?;
    let key = format!("{}{filename}", stage_info.key_prefix);
    let (url, sas_token) = resolve_url_and_token(stage_info, &key)?;
    ...
    let digest = get_header(headers, AZURE_META_SFC_DIGEST)?;
    let encryption_data_str = get_header(headers, AZURE_META_ENCRYPTIONDATA)?;
    let mat_desc_str = get_header(headers, AZURE_META_MATDESC)?;
    ...
    let material_desc: MaterialDescription =
        serde_json::from_str(&mat_desc_str).context(azure_download_error::DeserializationSnafu)?;
...
/// Upload encrypted data to Azure with retry logic.
async fn upload_to_azure(
    ...
    let encryption_data = build_encryption_metadata_json(&encryption_result.metadata);
    let encryption_data_str =
        serde_json::to_string(&encryption_data).context(azure_upload_error::SerializationSnafu)?;
    ...
    azure_request_with_retry(
        || {
            client
                ...
                .header(AZURE_META_ENCRYPTIONDATA, &encryption_data_str)
                .header(AZURE_META_MATDESC, &mat_desc)
                .body(data.clone())
        },
        Method::PUT,
    )
...
fn get_header(
    ...
    name: &str,
) -> Result<String, AzureDownloadError> {
    headers
        .get(name)
