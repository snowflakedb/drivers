...
impl ConfigResolver {
    ...
    pub fn resolve(
        explicit: &HashMap<String, Setting>,
    ) -> Result<HashMap<String, Setting>, ConfigError> {
        let paths = crate::config::path_resolver::get_config_paths()?;
        Self::resolve_with_paths(explicit, &paths)
    }

    /// Same as `resolve` but with explicit config file paths (for testing).
...
"#,
        ...
        if let Some(Setting::String(user)) = resolved.get("user") {
...
"#,
        ...
        if let Some(Setting::String(protocol)) = resolved.get("protocol") {
...
"#,
        ...
        assert_eq!(get_str(&resolved, "account"), Some("explicit_acct"));
        assert_eq!(get_str(&resolved, "database"), Some("conn_db"));
        assert_eq!(get_str(&resolved, "user"), Some("config_user"));
        assert_eq!(get_str(&resolved, "warehouse"), Some("config_wh"));
        assert_eq!(get_str(&resolved, "protocol"), Some("https"));
    }
}
