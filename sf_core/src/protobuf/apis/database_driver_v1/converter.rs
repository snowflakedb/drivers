...
/// code is the user-facing error number) and for ordinary login/authentication
/// failures, where the raw GS code (e.g. `390100` for bad credentials) is
/// surfaced as `vendor_code`, matching the legacy Python connector's behavior
/// (SNOW-3775156) — a code of `-1` means the server omitted or sent a
/// non-numeric code, in which case no vendor_code is surfaced and callers fall
/// back to their default errno.
///
/// Reauth-shaped `Login` failures (`RestError::LoginError` with
/// `reauthentication_required: true`) also pass through the generic arm below
/// and get a `vendor_code` populated — harmlessly redundant, not incorrect:
/// `client_api.py`'s `_convert_application_error` checks the dedicated
/// `ReauthenticationRequiredError` message (see `to_driver_error`) first and
/// takes that branch regardless of what `vendor_code` also contains.
/// `MasterTokenExpired` is unaffected either way — it's a distinct `ApiError`
/// variant this function never matches on.
///
...
mod tests {
    ...
    #[test]
    fn login_error_reauth_flag_does_not_prevent_generic_vendor_code() {
        // Reauth-shaped Login failures still pass through the generic
        // Login arm (SNOW-3775156) and get a vendor_code populated —
        // harmlessly, not incorrectly: client_api.py's
        // _convert_application_error checks the dedicated
        // ReauthenticationRequiredError message (see
        // login_reauth_flag_constructs_reauth_error_type_not_login_error
        // below) before it ever looks at vendor_code, so this value is
        // simply unused on that path. Unlike MasterTokenExpired (a
        // distinct ApiError variant extract_vendor_info never matches at
        // all — see master_token_expired_does_not_populate_generic_vendor_code
        // above), there's no equivalent hard exclusion for reauth-shaped
        // Login, by design: the exclusion happens at the class-selection
        // layer in Python, not by hiding vendor_code here.
