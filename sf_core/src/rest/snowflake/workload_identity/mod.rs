...
impl AttestationError {
    pub(crate) fn is_fips_mode_unavailable(&self) -> bool {
        match self {
            Self::CryptoProvider { source, .. } => {
                matches!(
                    source.as_ref(),
                    crate::tls::error::TlsError::FipsModeUnavailable { .. }
                )
            }
            Self::AwsAttestation { source, .. } => matches!(
                source,
                aws::AwsAttestationError::SdkHttpClient { source, .. }
                    if matches!(
                        source.as_ref(),
                        crate::tls::error::TlsError::FipsModeUnavailable { .. }
                    )
