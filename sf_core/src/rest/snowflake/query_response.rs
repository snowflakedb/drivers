...
/// scanning the map.
///
/// Accepted value forms: native `true`/`1` (whatever typed shape GS's JSON
/// sent), or the equivalent strings `"true"` (case-insensitive) and `"1"` for
/// any caller-supplied override. JDBC additionally accepts `"on"`; we don't
/// currently, since GS doesn't emit that form.
///
