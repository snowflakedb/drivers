...
/// DSN, connection parameters, or the server. When set to `true` or `1`, GCS
/// object URLs use the virtual-host style (`{bucket}.storage.googleapis.com`)
/// even when the stage payload omits `useVirtualUrl`.
