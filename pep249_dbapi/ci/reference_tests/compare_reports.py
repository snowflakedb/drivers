...
def filter_universal_by_reference_nodeids(
    universal_outcomes: Dict[str, str], reference_nodeids: Set[str]
) -> Dict[str, str]:
...
def categorize_test_outcomes(outcomes: Dict[str, str]) -> Dict[str, Set[str]]:
    ...
    categories = {"passed": set(), "failed": set(), "skipped": set()}
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare test results between universal and reference drivers"
    )
    parser.add_argument("--py", required=True, help="Python version label, e.g. 3.12")
    parser.add_argument(
        "--os", required=True, help="OS version label, e.g. ubuntu-latest"
    )
    parser.add_argument(
        "--universal", required=True, help="Path to universal driver JSON report"
    )
    parser.add_argument(
        "--reference", required=True, help="Path to reference driver JSON report"
    )
    parser.add_argument(
        "--summary", default="", help="Path to GITHUB_STEP_SUMMARY (optional)"
    )
    parser.add_argument(
        "--fail-on-regressions",
        type=int,
        default=0,
        help="1 to exit nonzero if regressions exist",
    )
    ...
    universal_outcomes = filter_universal_by_reference_nodeids(
        all_universal_outcomes, reference_nodeids
    )
    ...
    # Analyze differences
    regressions_from_pass = (
        reference_passed & universal_failed
    )  # Reference passed, universal failed
    regressions_from_fail = (
        reference_failed & universal_passed
    )  # Reference failed, universal passed
    ...
    comparison_details = "".join(
        [
            format_test_list(
                "Regressions from passing (ref ✅ / universal ❌)",
                regressions_from_pass,
            ),
            format_test_list(
                "Regressions from failing (ref ❌ / universal ✅)",
                regressions_from_fail,
            ),
