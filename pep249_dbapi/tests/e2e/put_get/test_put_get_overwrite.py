...
def test_should_overwrite_file_when_overwrite_is_set_to_true(connection):
    ...
    with connection.cursor() as cursor:
        stage_name = create_temporary_stage(cursor, "TEST_PUT_GET_OVERWRITE_TRUE")

        original_upload_result = upload_file_to_stage(
            cursor,
            stage_name,
            original_file_path,
            auto_compress=False,
        ...
        assert original_upload_result[6] == "UPLOADED"
...
def test_should_not_overwrite_file_when_overwrite_is_set_to_false(connection):
    ...
    # Given File is uploaded to stage
    with connection.cursor() as cursor:
        stage_name = create_temporary_stage(cursor, "TEST_PUT_GET_OVERWRITE_FALSE")
