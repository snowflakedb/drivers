...
def verify_login_error(exception):
    ...
    if NEW_DRIVER_ONLY("BC#4"):
        assert isinstance(
            exception.value.api_error_pb, DriverException
        ), f"Expected DriverException, got: {type(exception.value)}"
        assert (
            exception.value.error.WhichOneof("error_type") == "login_error"
        ), "Expected login error"
        assert (
            exception.value.error.login_error.code != 0
        ), "Login error code should not be zero"
        assert (
            exception.value.error.login_error.message.strip() != ""
    ...
    if OLD_DRIVER_ONLY("BC#4"):
        ...
        assert isinstance(
            exception.value, snowflake.connector.errors.DatabaseError
        ...
        assert (
            "jwt" in error_msg or "token" in error_msg or "invalid" in error_msg
