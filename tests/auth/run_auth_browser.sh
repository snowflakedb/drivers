...
#
# Usage:
#   DOCKER_IMAGE=<image> ./tests/auth/run_auth_browser.sh <python|odbc|jdbc|nodejs> [mode] [wrapper args...]
#
...
#
# If the second argument is not exactly universal or reference, it is treated
# as the first wrapper argument and mode stays universal.
#
# Everything after the mode is passed through as extra arguments. Only the
# Node.js wrapper consumes those arguments (a test path relative to nodejs/,
# plus any vitest flags); the other wrappers ignore them.
#
# Examples:
...
#   DOCKER_IMAGE=<image> ./tests/auth/run_auth_browser.sh nodejs tests/e2e/authentication/oauth.test.ts
#   DOCKER_IMAGE=<image> ./tests/auth/run_auth_browser.sh nodejs reference tests/e2e/authentication/oauth.test.ts -t "should authenticate"
#
...
if [ -z "$WRAPPER" ] || [ ! -f "${SCRIPT_DIR}/auth_browser_${WRAPPER}.sh" ]; then
    echo "Usage: $0 <python|odbc|jdbc|nodejs> [universal|reference] [wrapper args...]" >&2
    exit 1
fi
shift

if [ "${1:-}" = "universal" ] || [ "${1:-}" = "reference" ]; then
    AUTH_BROWSER_MODE="$1"
    shift
else
    AUTH_BROWSER_MODE=universal
fi
