...
# Script to generate CATCH tests for ODBC functions using Claude AI
# Usage: ./generate_catch_tests.sh <ODBC_FUNCTION_NAME>
#
# Example: ./generate_catch_tests.sh SQLDriverConnect
...
TEMPLATE_FILE="${SCRIPT_DIR}/catch_test_prompt_template.md"
GHERKIN_DIR="${ROOT_DIR}/tests/definitions/odbc/vibe"
OUTPUT_DIR="${ROOT_DIR}/odbc_tests/tests/vibe"
...
run_claude() {
    ...
    local prompt_dir="${OUTPUT_DIR}/${function_name}"
    mkdir -p "$prompt_dir"
    ...
    echo ""
    echo -e "${YELLOW}Running Claude AI to generate CATCH tests...${NC}"
    echo ""
    
    # Call sf ai claude with the prompt
    if sf ai claude -- "Generate CATCH tests based on this prompt: @${prompt_file}"; then
        echo ""
...
main() {
    ...
    echo "========================================="
    echo "ODBC CATCH Test Generator"
    echo "========================================="
    echo ""
