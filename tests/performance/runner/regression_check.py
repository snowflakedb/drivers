...
from runner.benchstore_baseline import get_main_baseline
from runner.local_compare import _is_main_result_file
...
def read_pr_medians(results_dir: Path, driver: str, driver_type: str) -> dict[str, float]:
    ...
    for test_dir in sorted(search_dir.iterdir()):
        ...
        csv_files = [
            f for f in test_dir.glob("*.csv") if _is_main_result_file(f)
        ]
        if not csv_files:
            continue
...
def _rerun_test(
    ...
    main_files = [f for f in result_files if _is_main_result_file(f)]
    if not main_files:
