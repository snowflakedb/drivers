...
Feature: ODBC SQLColAttributes function behavior (ODBC 2.x)
  ...
  @odbc_e2e
  Scenario: SQLColAttributes returns SQL_TYPE_DATE for DATE column via SQL_COLUMN_TYPE.
    Given A table with a DATE column is queried
    When SQLColAttributes is called with SQL_COLUMN_TYPE
    Then The call should succeed and return the concise type SQL_TYPE_DATE

  @odbc_e2e
  Scenario: SQLColAttributes returns SQL_TYPE_TIME for TIME column via SQL_COLUMN_TYPE.
    Given A table with a TIME column is queried
    When SQLColAttributes is called with SQL_COLUMN_TYPE
    Then The call should succeed and return the concise type SQL_TYPE_TIME

  @odbc_e2e
  Scenario: SQLColAttributes returns SQL_TYPE_TIMESTAMP for TIMESTAMP_NTZ column via SQL_COLUMN_TYPE.
