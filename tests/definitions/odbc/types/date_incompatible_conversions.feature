@odbc
Feature: ODBC DATE incompatible C type conversions
  # Per the ODBC spec, SQL_TYPE_DATE can only be converted to
  # SQL_C_TYPE_DATE, SQL_C_TYPE_TIMESTAMP, SQL_C_CHAR, SQL_C_WCHAR,
  # SQL_C_BINARY (and SQL_C_DEFAULT).
  # All other C types are incompatible.
  # Expected SQLSTATE: 07006 (Restricted data type attribute violation)

  @odbc_e2e
  Scenario: should fail converting DATE to numeric C types
  ...
  @odbc_e2e
  Scenario: should fail converting DATE to additional numeric C types
    Given Snowflake client is logged in
    When Query "SELECT '2024-01-15'::DATE" is executed
  ...
  @odbc_e2e
  Scenario: should fail converting DATE to single-component interval C types
    Given Snowflake client is logged in
    When Query "SELECT '2024-01-15'::DATE" is executed
  ...
  @odbc_e2e
  Scenario: should fail converting DATE to compound interval C types
    Given Snowflake client is logged in
    When Query "SELECT '2024-01-15'::DATE" is executed
  ...
  @odbc_e2e
  Scenario: should fail converting DATE to SQL_C_TYPE_TIME
    Given Snowflake client is logged in
    When Query "SELECT '2024-01-15'::DATE" is executed
    Then SQL_C_TYPE_TIME conversion should fail with SQLSTATE 07006

  @odbc_e2e
  Scenario: should fail converting DATE to SQL_C_GUID
    Given Snowflake client is logged in
    When Query "SELECT '2024-01-15'::DATE" is executed
