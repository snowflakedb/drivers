@odbc
Feature: ODBC TIME incompatible C type conversions
  # Per the ODBC spec, SQL_TYPE_TIME can only be converted to
  # SQL_C_TYPE_TIME, SQL_C_TYPE_TIMESTAMP, SQL_C_CHAR, SQL_C_WCHAR,
  ...
  @odbc_e2e
  Scenario: should fail converting TIME to numeric C types
  ...
  @odbc_e2e
  Scenario: should fail converting TIME to additional numeric C types
    Given Snowflake client is logged in
    When Query "SELECT '14:30:45'::TIME" is executed
  ...
  @odbc_e2e
  Scenario: should fail converting TIME to single-component interval C types
    Given Snowflake client is logged in
    When Query "SELECT '14:30:45'::TIME" is executed
  ...
  @odbc_e2e
  Scenario: should fail converting TIME to compound interval C types
    Given Snowflake client is logged in
    When Query "SELECT '14:30:45'::TIME" is executed
  ...
  @odbc_e2e
  Scenario: should fail converting TIME to SQL_C_TYPE_DATE
    Given Snowflake client is logged in
    When Query "SELECT '14:30:45'::TIME" is executed
    Then SQL_C_TYPE_DATE conversion should fail with SQLSTATE 07006

  @odbc_e2e
  Scenario: should fail converting TIME to SQL_C_GUID
    Given Snowflake client is logged in
    When Query "SELECT '14:30:45'::TIME" is executed
