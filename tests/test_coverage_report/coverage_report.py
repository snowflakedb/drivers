...
class CoverageReportGenerator:
    ...
    def __init__(self, workspace_root: str):
        ...
        self.breaking_change_handler = BreakingChangesHandler(
            self.validator, self.feature_parser
        )
    ...
    def run_validator(self) -> Dict:
        ...
        try:
            ...
            if result.returncode not in [
                0,
                1,
            ]:  # 1 is expected when orphaned tests are found
    ...
    def _parse_validator_output(self, output: str) -> Dict:
                ...
                if line.startswith("In method ") and " (scenario: " in line:
                    ...
                    try:
                        ...
                        if current_language in features[current_feature]["languages"]:
                            features[current_feature]["languages"][current_language][
                                "scenarios"
                            ][scenario_name] = False
        ...
        for feature_name, feature_data in features.items():
            scenarios_with_annotations = self.get_feature_scenarios_with_annotations(
                feature_data["path"]
            )
            for lang, lang_data in feature_data["languages"].items():
                if lang_data[
                    "implemented"
                ]:  # If overall language implementation passed
    ...
    def get_feature_scenarios_with_annotations(
    ...
    def parse_breaking_change_descriptions(
        self, driver: str = "odbc"
    ) -> Dict[str, str]:
    ...
    def extract_breaking_change_from_test_files(
        self, driver: str = "odbc", features: Dict = None
        ...
        """
        return self.breaking_change_handler.get_breaking_change_test_mappings(
            driver, features
        )
    ...
    def is_scenario_todo_for_driver(self, scenario_info: Dict, driver: str) -> bool:
        ...
        has_driver_tag = (
            f"@{driver_lower}" in tag_strings
            or f"@{driver_lower}_e2e" in tag_strings
            or f"@{driver_lower}_int" in tag_strings
        )
        if has_driver_tag:
            return False
        ...
        # Handle both old format (@driver) and new format (@driver_e2e, @driver_int)
        feature_has_driver = (
            f"@{driver_lower}" in feature_tag_strings
            or f"@{driver_lower}_e2e" in feature_tag_strings
            or f"@{driver_lower}_int" in feature_tag_strings
        )
    ...
    def _convert_validator_data_to_features(self, validator_data: Dict) -> Dict:
        ...
        for validation_result in validator_data.get("validation_results", []):
            ...
            features[feature_name] = {"path": feature_file_path, "languages": {}}
            ...
            for validation in validation_result.get("validations", []):
                ...
                if test_file_path.startswith("./"):
                    ...
                    relative_path = Path(test_file_path).relative_to(
                        self.workspace_root
                    )
    ...
    def _get_integration_test_file_path(
        self, language: str, feature_file_path: str
    ) -> Path:
        ...
        if language.lower() == "core":
            ...
            if subdir:
                return (
                    self.workspace_root
                    / f"sf_core/tests/integration/{subdir}/{feature_name}.rs"
                )
            else:
                return (
                    self.workspace_root / f"sf_core/tests/integration/{feature_name}.rs"
                )
            ...
            if subdir:
                return (
                    self.workspace_root
                    / f"pep249_dbapi/tests/integ/{subdir}/test_{feature_name}.py"
                )
            else:
                return (
                    self.workspace_root
                    / f"pep249_dbapi/tests/integ/test_{feature_name}.py"
                )
            ...
            if subdir:
                return (
                    self.workspace_root
                    / f"odbc_tests/tests/integration/{subdir}/{feature_name}.cpp"
                )
            else:
                return (
                    self.workspace_root
                    / f"odbc_tests/tests/integration/{feature_name}.cpp"
                )
        else:
            # Fallback - return a non-existent path
            return self.workspace_root / "non_existent_path"

    def extract_test_methods_with_lines(
        self, file_path: str, scenario_names: List[str]
    ) -> Dict[str, int]:
        """Extract test method line numbers from a test file for given scenarios."""
        return self.feature_parser.extract_test_methods_with_lines(
            file_path, scenario_names
        )
    ...
    def _method_matches_scenario(self, method_name: str, scenario_name: str) -> bool:
        ...
        return (
            method_normalized == normalize(scenario_name)
            or method_normalized == normalize(scenario_snake)
            or method_normalized == normalize(scenario_pascal)
            or method_normalized == normalize(scenario_test_method)
        )

    def _get_breaking_change_ids_for_scenario(
        self, scenario_info: Dict, driver: str, features: Dict = None
    ) -> List[str]:
        """Get all Breaking Change IDs associated with a scenario by looking up test implementations."""
        return self.breaking_change_handler.get_breaking_change_ids_for_scenario(
            scenario_info, driver, features
        )
    ...
    def generate_coverage_table(self, features: Dict) -> str:
        ...
        all_names = list(features.keys()) + [
            folder.replace("_", " ").title() for folder in features_by_folder.keys()
        ]
        feature_width = max(
            len("Feature"), max(len(name) for name in all_names) if all_names else 0
        )
        ...
        for folder, folder_features in sorted(features_by_folder.items()):
            ...
            total_width = feature_width + sum(
                lang_width + 3 for _ in languages
            )  # +3 for " | "
            folder_header = f"=== {folder_display} " + "=" * (
                total_width - len(folder_display) - 4
            )
            table.append(folder_header)

            # Features in this folder
            for feature_name, feature_data in folder_features.items():
                formatted_name = self.format_feature_name(
                    feature_name, feature_data["path"]
                )
    ...
    def _generate_coverage_table_html(
        self, features: Dict, languages: List[str]
    ) -> str:
        ...
        header_cells = ["<th>Feature</th>"] + [
            f"<th>{lang.title()}</th>" for lang in languages
        ]
        ...
        for folder, folder_features in sorted(features_by_folder.items()):
            ...
            for feature_name, feature_data in folder_features.items():
                ...
                scenarios_with_annotations = (
                    self.get_feature_scenarios_with_annotations(feature_data["path"])
                )
                scenarios = (
                    [s["name"] for s in scenarios_with_annotations]
                    if scenarios_with_annotations
                    else self.get_feature_scenarios(feature_data["path"])
                )
                ...
                feature_cells = [
                    f'<td><div class="feature-name" onclick="toggleFeature(\'{feature_id}\')">{formatted_name}</div></td>'
                ]
                ...
                for lang in languages:
                    ...
                    for scenario_info in scenarios_with_annotations:
                        is_todo_for_driver = self.is_scenario_todo_for_driver(
                            scenario_info, lang
                        )
                        breaking_change_driver_check = (
                            self.get_language_data_key(lang).lower()
                            if lang == "core"
                            else lang.lower()
                        )

                        # Check if there are actual Breaking Change implementations for this driver and scenario
                        breaking_change_ids_for_this_driver = (
                            self._get_breaking_change_ids_for_scenario(
                                scenario_info, breaking_change_driver_check, features
                            )
                        )
                        is_breaking_change_for_this_driver = bool(
                            breaking_change_ids_for_this_driver
                        )
                        has_driver_tag = self._has_driver_tag_in_scenario(
                            scenario_info, lang.lower()
                        )

                        if (
                            is_todo_for_driver
                            or is_breaking_change_for_this_driver
                            or has_driver_tag
                        ):
                            ...
                            if is_todo_for_driver:
                                ...
                                if data_key in feature_data["languages"]:
                                    ...
                                    scenario_name = scenario_info["name"]
                                    if (
                                        "scenarios" in lang_data
                                        and scenario_name in lang_data["scenarios"]
                                    ):
                                        scenario_implemented = lang_data["scenarios"][
                                            scenario_name
                                        ]
                                    elif lang_data[
                                        "status"
                                    ] == "✅" and self._has_driver_tag_in_scenario(
                                        scenario_info, lang.lower()
                                    ):
                    ...
                    if not has_relevant_scenarios:
                        ...
                        feature_cells.append(
                            '<td><div class="test-status"><span class="status-na">-</span></div></td>'
                        )
                    elif has_expected_scenarios:
                        # Has expected (in progress) scenarios
                        feature_cells.append(
                            '<td><div class="test-status"><span class="status-in-progress">TODO</span></div></td>'
                        )
                    elif has_failed_scenarios:
                        # Has failed scenarios
                        feature_cells.append(
                            '<td><div class="test-status"><span class="status-fail">✗</span></div></td>'
                        )
                    elif has_implemented_scenarios or has_breaking_change_scenarios:
                        # All scenarios are implemented or Breaking Change
                        feature_cells.append(
                            '<td><div class="test-status"><span class="tick-icon">✓</span></div></td>'
                        )
                    else:
                ...
                category_rows.append(
                    f'<tr class="feature-row">{"".join(feature_cells)}</tr>'
                )

                # Individual test rows - collect in feature content
                feature_test_rows = []
                for i, scenario_info in enumerate(
                    scenarios_with_annotations
                    if scenarios_with_annotations
                    else [
                        {
                    ...
                    breaking_change_info = (
                        scenario_info.get("breaking_change_info")
                        if isinstance(scenario_info, dict)
                        else None
                    )
                    expected_drivers = (
                        scenario_info.get("expected_drivers", [])
                        if isinstance(scenario_info, dict)
                        else []
                    )
                    tags = (
                        scenario_info.get("tags", [])
                        if isinstance(scenario_info, dict)
                        else []
                    )
                    ...
                    test_level_label = (
                        '<span class="test-level-integration">Integration</span>'
                        if has_int_tag
                        else '<span class="test-level-e2e">E2E</span>'
                    )
                    ...
                    for lang in languages:
                        ...
                        if data_key in feature_data["languages"]:
                            ...
                            if self._has_driver_tag_in_tags(tags, lang.lower()):
                                if (
                                    "scenarios" in lang_data
                                    and scenario in lang_data["scenarios"]
                                ):
                            ...
                            if is_todo_for_driver:
                                ...
                                if breaking_change_ids_for_link:
                                    ...
                                    for (
                                        breaking_change_id
                                    ) in breaking_change_ids_for_link:
                                        breaking_change_section_id = f'breaking_change-{breaking_change_driver_check}-{breaking_change_id.lower().replace("#", "")}'
                                        breaking_change_items.append(
                                            f'<li><a href="#" onclick="navigateToBreakingChange(\'{breaking_change_section_id}\'); hideBreakingChangePopup(\'{popup_id}\'); return false;" class="breaking_change-popup-link">{breaking_change_id}</a></li>'
                                        )
                                    ...
                                    for (
                                        ...
                                        if breaking_change_id.startswith("BC#"):
                                            number = breaking_change_id[
                                                3:
                                            ]  # Remove 'BC#' prefix
                                            breaking_change_section_id = f'breaking_change-{breaking_change_driver_check}-{breaking_change_id.lower().replace("#", "")}'
                                            breaking_change_links.append(
                                                f'<a href="#" onclick="navigateToBreakingChange(\'{breaking_change_section_id}\'); return false;" class="breaking_change-superscript-link">{number}</a>'
                                            )
                                ...
                                is_excluded = self.is_scenario_excluded_for_driver(
                                    scenario_info, lang
                                )
                                is_todo = self.is_scenario_todo_for_driver(
                                    scenario_info, lang
                                )
                    ...
                    category_rows.append(
                        f'<tr class="{row_class} feature-content" data-feature="{feature_id}">{test_cell}{"".join(status_cells)}</tr>'
                    )
                ...
                if "feature-row" in row:
                    rows.append(
                        row.replace(
                            ...
                            f'<tr class="feature-row" data-category="{folder_id}">',
                        )
                    )
                else:
                    rows.append(
                        row.replace('<tr class="', f'<tr class="').replace(
                            '">', f'" data-category="{folder_id}">'
                        )
                    )
    ...
    def _generate_language_coverage_html(
        self, features: Dict, languages: List[str]
    ) -> str:
        ...
        for lang in languages:
            ...
            for feature_name, feature_data in features.items():
                ...
                for scenario_info in scenarios_with_annotations:
                    ...
                    breaking_change_ids_for_this_driver = (
                        self._get_breaking_change_ids_for_scenario(
                            scenario_info, lang.lower(), features
                        )
                    )
                    is_breaking_change_for_this_driver = bool(
                        breaking_change_ids_for_this_driver
                    ...
                    has_driver_implementation = self._has_driver_tag_in_tags(
                        tags, lang.lower()
                    )
                    ...
                    is_relevant = (
                        has_driver_implementation
                        or is_breaking_change_for_this_driver
                        or is_todo_for_driver
                    )
                    ...
                    if is_relevant:
                        ...
                        if data_key in feature_data["languages"]:
                            lang_data = feature_data["languages"][data_key]
                            # Only consider a scenario implemented if it has the driver tag
                            if self._has_driver_tag_in_scenario(
                                scenario_info, lang.lower()
                            ):
                                if (
                                    "scenarios" in lang_data
            ...
            lang_coverage = (
                (implemented_count / expected_count) * 100 if expected_count > 0 else 0
            )
            coverage_class = (
                "coverage-high"
                if lang_coverage >= 80
                else "coverage-medium" if lang_coverage >= 60 else "coverage-low"
            )
            ...
            if expected_count == 0:
                ...
                test_cases_display = (
                    f"{implemented_count}/{expected_count} expected tests"
                )
                ...
                if scenarios_with_annotations:
                    ...
                    for scenario_info in e2e_scenarios + integration_scenarios:
                        ...
                        display_languages = [
                            lang
                            for lang in languages
                            if self.get_language_data_key(lang)
                            in feature_data["languages"]
                        ]
                        ...
                        for lang in sorted(display_languages):
                            ...
                            if has_int_tag:
                                ...
                                actual_file_path = self._get_integration_test_file_path(
                                    lang, feature_data["path"]
                                )
                                if actual_file_path and actual_file_path.exists():
                                    # Update the display path for integration tests
                                    relative_path = actual_file_path.relative_to(
                                        self.workspace_root
                                    )
                                ...
                                if lang_data["implemented"] and "path" in lang_data:
                                    actual_file_path = (
                                        self.workspace_root / lang_data["path"]
                                    )
                            ...
                            if is_todo_for_driver:
                                ...
                                for (
                                    breaking_change_id
                                ) in breaking_change_ids_for_this_driver:
                                    if breaking_change_id.startswith("BC#"):
                    ...
                    if scenarios:
                        ...
                        for scenario in scenarios:
                            ...
                            for lang in sorted(feature_data["languages"].keys()):
                                ...
                                if lang_data["implemented"] and "path" in lang_data:
                                    ...
                                    if file_path.exists():
                                        method_lines = (
                                            self.extract_test_methods_with_lines(
                                                str(file_path), [scenario]
                                            )
                                        )
                                        if method_lines and scenario in method_lines:
                                            scenario_implemented = True
                                            line_info = (
                                                f" (line {method_lines[scenario]})"
                                            )
                                ...
                                status_text = (
                                    "✅" if scenario_implemented else "❌ MISSING"
                                )
                        ...
                        for lang in sorted(feature_data["languages"].keys()):
                            ...
                            badge_class = (
                                "lang-implemented"
                                if lang_data["implemented"]
                                else "lang-failed"
                            )
                            status_text = (
                                "✅" if lang_data["implemented"] else "❌ MISSING"
                            )
    ...
    def _generate_test_level_section(
        self,
        section_title: str,
        scenarios: List,
        feature_data: Dict,
        ...
        for scenario_info in scenarios:
            ...
            display_languages = [
                lang
                for lang in languages
                if self.get_language_data_key(lang) in feature_data["languages"]
            ]
            for lang in sorted(display_languages):
                data_key = self.get_language_data_key(lang)
                lang_data = feature_data["languages"][data_key]
                ...
                if lang_data["implemented"] and "path" in lang_data:
                    ...
                    file_path = self.workspace_root / lang_data["path"]
                    if file_path.exists():
                        method_lines = self.extract_test_methods_with_lines(
                            str(file_path), [scenario]
                        )
                        if method_lines and scenario in method_lines:
                            scenario_implemented = True
                ...
                breaking_change_ids_for_this_driver = (
                    self._get_breaking_change_ids_for_scenario(
                        scenario_info, breaking_change_driver_check, feature_data
                    )
                )
                is_breaking_change_for_this_driver = bool(
                    breaking_change_ids_for_this_driver
    ...
    def _generate_missing_implementations_html(
        self, features: Dict, languages: List[str]
    ) -> str:
            ...
            for scenario_info in scenarios_with_annotations:
                ...
                for lang in languages:
                    ...
                    if is_todo:
                        ...
                        if (
                            not scenario_implemented
                            and not is_breaking_change_for_this_driver
                        ):
                            missing_by_driver[lang].append(
                                {
                                    "feature": feature_name,
                                    "scenario": scenario,
                                    "formatted_feature": self.format_feature_name(
                                        feature_name, feature_data["path"]
                                    ),
                                }
                            )
        ...
        for driver in sorted(missing_by_driver.keys()):
            ...
            for feature_name in sorted(by_feature.keys()):
                ...
                formatted_feature = next(
                    item["formatted_feature"]
                    for item in missing_items
                    if item["feature"] == feature_name
                )

                scenario_list = "\n                        ".join(
                    [f"<li>• {scenario}</li>" for scenario in sorted(scenarios)]
                )
    ...
    def _generate_breaking_change_tab_html(self, features: Dict) -> str:
        ...
        breaking_change_data = (
            self.breaking_change_handler.validator.get_breaking_change_data()
        )
        if breaking_change_data:
            breaking_change_by_language = breaking_change_data.get(
                "breaking_changes_by_language", {}
            )
            for language, breaking_change_list in breaking_change_by_language.items():
                if (
                    breaking_change_list
                ):  # Only add if there are actual Breaking Changes
        ...
        for feature_name, feature_data in features.items():
            ...
            for scenario_info in scenarios_with_annotations:
                ...
                if breaking_change_info:
                    drivers_with_breaking_change.add(
                        breaking_change_info["driver"].lower()
                    )
        ...
        for driver in sorted(drivers_with_breaking_change):
            ...
            breaking_change_descriptions = self.parse_breaking_change_descriptions(
                driver
            )

            # Extract Breaking Change test implementations for this driver
            breaking_change_test_mapping = self.extract_breaking_change_from_test_files(
                driver, features
            )
            ...
            for feature_name, feature_data in features.items():
                ...
                for scenario_info in scenarios_with_annotations:
                    ...
                    if (
                        breaking_change_info
                        and breaking_change_info["driver"].lower() == driver
                    ):
                        ...
                        feature_breaking_changes["Breaking Change_FROM_FEATURE"].append(
                            {
                                "feature": self.format_feature_name(
                                    feature_name, feature_data["path"]
                                ),
                                "scenario": scenario_info["name"],
            ...
            for breaking_change_id in sorted(all_breaking_changes):
                description = breaking_change_descriptions.get(
                    breaking_change_id, "No description available"
                )
                ...
                test_implementations = breaking_change_test_mapping.get(driver, {}).get(
                    breaking_change_id, []
                )
                ...
                for impl in test_implementations:
                    test_line_info = (
                        f"{impl['test_line']}" if impl.get("test_line") else "unknown"
                    )
                    ...
                    if impl.get("new_behaviour_file") and impl.get(
                        "new_behaviour_line"
                    ):
                        new_file = impl["new_behaviour_file"]
                        new_line = impl["new_behaviour_line"]
                        behaviour_lines.append(
                            f"<strong>New Behaviour:</strong> <code>{new_file}:{new_line}</code>"
                        )

                    # Old behaviour (OLD_DRIVER_ONLY)
                    if impl.get("old_behaviour_file") and impl.get(
                        "old_behaviour_line"
                    ):
                        old_file = impl["old_behaviour_file"]
                        old_line = impl["old_behaviour_line"]
                        behaviour_lines.append(
                            f"<strong>Old Behaviour:</strong> <code>{old_file}:{old_line}</code>"
                        )
                    ...
                    if not behaviour_lines:
                        ...
                        assertion_line = impl.get(
                            "assertion_line", impl.get("line_number", "unknown")
                        )
                        behaviour_lines.append(
                            f"<strong>Assertion:</strong> <code>{assertion_file}:{assertion_line}</code>"
                        )
                ...
                impl_html = (
                    "\n                    ".join(impl_items)
                    if impl_items
                    else "<li><em>No test implementations found</em></li>"
                )
                ...
                breaking_change_section = dedent(
                    f"""
            ...
            breaking_changes_html = (
                "\n            ".join(breaking_change_sections)
                if breaking_change_sections
                else "<p><em>No Breaking Changes found for this driver</em></p>"
            )
    ...
    def generate_html_report(self, features: Dict) -> str:
        ...
        missing_impl_html = self._generate_missing_implementations_html(
            features, languages
        )
        ...
        """
            ...
            missing_content=(
                ...
                else "<p>No missing implementations found! ✅</p>"
            ),
        )
        ...
        return self._create_html_document(
            "Universal Driver Test Coverage Report", body_content
        )
