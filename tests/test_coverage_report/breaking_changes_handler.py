...
class BreakingChangesHandler:
    ...
    def get_breaking_change_descriptions(self, driver: str = "odbc") -> Dict[str, str]:
        ...
        if not breaking_change_data:
            print(
                f"Warning: Could not get Breaking Change descriptions for {driver} from Rust validator"
            )
            return {}

        # Extract descriptions from the specific language's Breaking Changes
        breaking_changes_by_language = breaking_change_data.get(
    ...
    def get_breaking_change_test_mappings(
        ...
        if not breaking_change_data:
            print(
                f"Warning: Could not get Breaking Change test mappings for {driver} from Rust validator"
            )
            return {}
                ...
                for breaking_change_info in breaking_change_list:
                    ...
                    for impl in breaking_change_info["implementations"]:
                        ...
                        converted_impl = {
                            ...
                            "line_number": impl.get("new_behaviour_line")
                            or impl.get("old_behaviour_line", impl["test_line"]),
                            "assertion_file": impl.get("new_behaviour_file")
                            or impl.get("old_behaviour_file", impl["test_file"]),
                            "assertion_line": impl.get("new_behaviour_line")
                            or impl.get("old_behaviour_line", impl["test_line"]),
                        }

                        if impl.get("new_behaviour_file"):
                            converted_impl["new_behaviour_file"] = impl[
                                "new_behaviour_file"
                            ]
                            converted_impl["new_behaviour_line"] = impl[
                                "new_behaviour_line"
                            ]

                        if impl.get("old_behaviour_file"):
                            converted_impl["old_behaviour_file"] = impl[
                                "old_behaviour_file"
                            ]
                            converted_impl["old_behaviour_line"] = impl[
                                "old_behaviour_line"
                            ]
    ...
    def get_breaking_change_ids_for_scenario(
        ...
        breaking_change_test_mapping = self.get_breaking_change_test_mappings(
            driver, features
        )
        ...
        if driver in breaking_change_test_mapping:
            for breaking_change_id, implementations in breaking_change_test_mapping[
                driver
            ].items():
                for impl in implementations:
                    if self.feature_parser._method_matches_scenario(
                        impl["test_method"], scenario_name
                    ):
