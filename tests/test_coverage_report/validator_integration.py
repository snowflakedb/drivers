...
class ValidatorIntegration:
    ...
    def get_validator_data(self) -> Dict:
        ...
        try:
            validator_binary = (
                self.validator_path / "target" / "release" / "tests_format_validator"
            )
            ...
            result = subprocess.run(
                [
                    ...
                    "--features",
                    str(self.workspace_root / "tests" / "definitions"),
                    "--json",
                ],
                capture_output=True,
                text=True,
    ...
    def get_features_data(self) -> Dict:
        ...
        return self._parse_validator_output(
            validator_data.get("validation_results", {})
        )
    ...
    def _parse_validator_output(self, validation_results: Dict) -> Dict:
        ...
        for language_name, language_validation in validation_results.items():
            ...
            for feature_name, feature_validation in language_validation.get(
                "features", {}
            ).items():
