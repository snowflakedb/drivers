...
class HTMLGenerator:
    ...
    def generate_html_report(
        self, features: Dict, breaking_change_handler, feature_parser
    ) -> str:
        ...
        overview_content = (
            self._generate_coverage_table_html(
                features, languages, breaking_change_handler, feature_parser
            )
            + "\n\n"
            + self._generate_language_coverage_html(features, languages)
        )
        detailed_content = self._generate_detailed_breakdown_html(
            features, feature_parser
        )
        missing_impl_html = self._generate_missing_implementations_html(
            features, languages, feature_parser
        )
        breaking_change_content = self._generate_breaking_change_tab_html(
            features, breaking_change_handler
        )

        # Generate complete HTML document
        return self._create_html_document(
            "Test Coverage Report",
            self._generate_tabbed_interface(
                overview_content,
                detailed_content,
                missing_impl_html,
    ...
    def generate_coverage_table(self, features: Dict) -> str:
        ...
        feature_width = (
            max(
                max(
                    len(self._format_feature_name_simple(name))
    ...
    def _generate_missing_implementations_html(
        self, features: Dict, languages: List[str], feature_parser
    ) -> str:
    ...
    def _generate_breaking_change_tab_html(
        self, features: Dict, breaking_change_handler
    ) -> str:
