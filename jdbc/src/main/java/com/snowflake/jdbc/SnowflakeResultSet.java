...
public class SnowflakeResultSet implements ResultSet {
  ...
  public String getString(int columnIndex) throws SQLException {
    return getConverted(columnIndex, ArrowVectorConverter::toString);
  }

  @Override
  public boolean getBoolean(int columnIndex) throws SQLException {
    return getConverted(columnIndex, ArrowVectorConverter::toBoolean);
  }

  @Override
  public byte getByte(int columnIndex) throws SQLException {
    return getConverted(columnIndex, ArrowVectorConverter::toByte);
  }

  @Override
  public short getShort(int columnIndex) throws SQLException {
    return getConverted(columnIndex, ArrowVectorConverter::toShort);
  }

  @Override
  public int getInt(int columnIndex) throws SQLException {
    return getConverted(columnIndex, ArrowVectorConverter::toInt);
  }

  @Override
  public long getLong(int columnIndex) throws SQLException {
    return getConverted(columnIndex, ArrowVectorConverter::toLong);
  }

  @Override
  public float getFloat(int columnIndex) throws SQLException {
    return getConverted(columnIndex, ArrowVectorConverter::toFloat);
  }

  @Override
  public double getDouble(int columnIndex) throws SQLException {
    return getConverted(columnIndex, ArrowVectorConverter::toDouble);
  }
  ...
  public byte[] getBytes(int columnIndex) throws SQLException {
    return getConverted(columnIndex, ArrowVectorConverter::toBytes);
  }
  ...
  public Object getObject(int columnIndex) throws SQLException {
    return getConverted(columnIndex, ArrowVectorConverter::toObject);
  }
  ...
  public BigDecimal getBigDecimal(int columnIndex) throws SQLException {
    return getConverted(columnIndex, ArrowVectorConverter::toBigDecimal);
  }
  ...
  public boolean isLast() throws SQLException {
    checkClosed();
    if (afterLast || currentRow < 0) {
      return false;
    }
    if (!onLastRow && currentRowInBatch == currentBatchRowCount - 1) {
      prefetchNextBatchForIsLast();
    }
    return onLastRow && !afterLast;
  }
  ...
  private ArrowVectorConverter getConverter(int columnIndex) throws SQLException {
    ...
    try {
      FieldVector vector = getActiveRoot().getVector(columnIndex - 1);
  ...
  private <T> T getConverted(int columnIndex, ConverterFunction<T> converterFn)
      throws SQLException {
    checkClosed();
    checkRowPosition();
    ...
    try {
      T value = converterFn.convert(converter, currentRowInBatch);
      wasNull = converter.isNull(currentRowInBatch);
      return value;
    } catch (SFException e) {
      throw new SQLException("Cannot convert column " + columnIndex, e);
    }
  }
  ...
  private void ensureSchemaInitialized() throws SQLException {
    ...
    List<Field> fields = getActiveRoot().getSchema().getFields();
  ...
  private boolean fetchNextRow() throws IOException, SQLException {
    if (afterLast) {
      return false;
    }
    if (!hasLoadedBatch || currentRowInBatch + 1 >= currentBatchRowCount) {
      if (hasPrefetchedBatch) {
        setCurrentRoot(prefetchedRoot, false);
        prefetchedRoot = null;
        hasPrefetchedBatch = false;
      } else {
        ...
        setCurrentRoot(nextRoot, false);
      }
      currentBatchRowCount = currentRoot.getRowCount();
      currentRowInBatch = 0;
      hasLoadedBatch = true;
      converterCache.clear();
      ensureSchemaInitialized();
  ...
  private void prefetchNextBatchForIsLast() throws SQLException {
    if (hasPrefetchedBatch || afterLast) {
      return;
    }
    VectorSchemaRoot snapshot = copyVectorSchemaRoot(getActiveRoot());
    setCurrentRoot(snapshot, true);
    converterCache.clear();
    try {
      VectorSchemaRoot nextRoot = loadNextNonEmptyBatch();
      if (nextRoot == null) {
      ...
      prefetchedRoot = nextRoot;
      hasPrefetchedBatch = true;
      onLastRow = false;
    } catch (IOException e) {
  ...
  private VectorSchemaRoot copyVectorSchemaRoot(VectorSchemaRoot source) {
  ...
  private VectorSchemaRoot getActiveRoot() throws SQLException {
    if (currentRoot != null) {
      return currentRoot;
    }
    try {
  ...
  private void setCurrentRoot(VectorSchemaRoot root, boolean owned) {
