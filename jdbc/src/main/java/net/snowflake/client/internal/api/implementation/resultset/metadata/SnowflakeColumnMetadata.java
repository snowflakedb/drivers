...
public class SnowflakeColumnMetadata implements Serializable {
  ...
  public SnowflakeColumnMetadata(ColumnMetadata colMetadata, boolean jdbcTreatDecimalAsInt) {
    ...
    //  `ColumnMetadata` now carries a nested `fields` list, but it is not read here yet,
    //  so this stays empty and `getSnowflakeType` treats every column as non-structured.
