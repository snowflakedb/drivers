...
class BuildHook(BuildHookInterface):
    ...
    def _build_core(self) -> None:
        ...
        Produces ``sf_core_python.<ext>`` (a native Python extension module) and
        ``sf_core_python.pyi`` (typing stubs generated by parsing lib.rs) that replace the
        old ctypes-loaded libsf_core cdylib.
        """
        ...
        with dir_ctx as build_dir:
            # PyO3 must link against the interpreter running this install, not
            ...
            self._generate_stubs(cargo_manifest, Path(build_dir), target_dir, cargo_env)
    ...
    def _generate_stubs(
        ...
        The stub_gen binary only needs the ``stub-gen`` feature (pulls in syn)
        and does not link against libpython or the extension module code.
        """
        if os.environ.get("SKIP_STUB_GENERATION", "").lower() in self.POSITIVE_VALUES:
            return
        ...
        stub_path = target_dir / "sf_core_python.pyi"
        if not stub_path.is_file():
            raise RuntimeError(f"Expected stub file was not generated: {stub_path}")
        print(f"Generated Python stub: {stub_path}")