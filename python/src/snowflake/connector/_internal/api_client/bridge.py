...
class ProtoTransport:
    """Serialize a protobuf RPC into a blocking FFI call or an async wrapper."""

    async def handle_message(self, api: str, method: str, message: bytes) -> tuple[int, bytes]:
        return await asyncio.to_thread(self.handle_message_sync, api, method, message)

    def handle_message_sync(self, api: str, method: str, message: bytes) -> tuple[int, bytes]:
