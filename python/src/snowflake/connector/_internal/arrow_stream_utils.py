...
def collect_arrow_table(
    ...
    When *force_return_table* is ``True`` (the default) an empty result set
    produces a properly-typed empty table.  When ``False``, an empty result
    set returns ``None``.

    When *description* is provided the empty-table schema is normalised via
