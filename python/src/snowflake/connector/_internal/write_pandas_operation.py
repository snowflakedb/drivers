...
class WritePandasOperation:
    ...
    def _build_create_stage_sql(self, name: str) -> _ParameterizedSql:
        cfg = self._cfg
        mapped = VALID_COMPRESSIONS_MAP[cfg.compression]
        fmt_opts = [f"TYPE=PARQUET COMPRESSION={mapped}"]
        if cfg.binary_as_text_false_on_stage:
            fmt_opts.append("BINARY_AS_TEXT=FALSE")
        return _ParameterizedSql(f"CREATE TEMPORARY STAGE IDENTIFIER(?) FILE_FORMAT=({' '.join(fmt_opts)})", (name,))

    def _build_create_file_format_sql(self, name: str) -> _ParameterizedSql:
        cfg = self._cfg
        mapped = VALID_COMPRESSIONS_MAP[cfg.compression]
        ...
        return _ParameterizedSql(" ".join(parts), (name,))
