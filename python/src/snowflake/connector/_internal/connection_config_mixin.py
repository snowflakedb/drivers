...
class ConnectionConfigMixin:
    ...
    def from_kwargs(cls: type[_Self], **kwargs: Any) -> _Self:
        ...
        # Legacy connector accepted list[str] for workload_identity_impersonation_path.
        # Rust expects a comma-separated str; coerce here before the field resolves.
        # Empty list coerces to None (same semantics as absent — legacy treats [] as falsy).
        impersonation_path_key = next((k for k in kwargs if k.lower() == "workload_identity_impersonation_path"), None)
        if impersonation_path_key is not None:
