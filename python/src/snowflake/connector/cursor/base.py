...
from snowflake.connector._internal.errorcode import ER_CURSOR_IS_CLOSED
...
class SnowflakeCursorBase(abc.ABC):
    ...
    def _fetchone(self) -> Row | DictRow | None:
        ...
        """
        iterator = self._init_row_iterator_if_needed()
        try:
            row = next(iterator)
            self._rownumber += 1
            return row
    ...
    def _init_row_iterator_if_needed(self) -> Iterator[Row]:
