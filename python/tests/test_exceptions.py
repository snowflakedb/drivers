...
class TestExtractErrorDetail:
    ...
    def test_authentication_error_returns_detail_unchanged(self):
        # `detail` already embeds the GS code (see master_token_terminal_detail
        # on the Rust side); this must pass through as-is, not gain a "(code=N)"
        # suffix — that would defeat _append_detail's exact-substring dedup and
        # print the code multiple times in the assembled message.
        from snowflake.connector._internal.api_client.client_api import _extract_error_detail

        driver_exc = ProtoDriverException(
            error=ProtoDriverError(
                auth_error=ProtoAuthenticationError(
        ...
        result = _extract_error_detail(driver_exc)
        assert result == "Master token can never be renewed - full re-authentication required (GS code 390114)."
        assert result.count("390114") == 1
...
class TestConvertProtoError:
    ...
    def test_application_exception_reauth_shaped_auth_error_constructs_reauthentication_request(self, code):
        ...
        # AuthenticationError.reauthentication_required discriminant.
        from snowflake.connector._internal.protobuf_gen.proto_exception import (
            ProtoApplicationException,
        )
        ...
        driver_exc = ProtoDriverException(
            ...
            error=ProtoDriverError(
                auth_error=ProtoAuthenticationError(
                    detail="Master token can never be renewed",
                    reauthentication_required=True,
                ),
            ),
        )
    ...
    def test_application_exception_reauth_shaped_auth_error_without_code_still_constructs_reauthentication_request(
        ...
        from snowflake.connector._internal.status_codes import STATUS_TO_ERRNO
        ...
        result = _proto_to_public_error(proto_exc)
        assert isinstance(result, ReauthenticationRequest)
        assert result.errno == STATUS_TO_ERRNO[STATUS_CODE_AUTHENTICATION_ERROR]
        assert result.sqlstate == "08001"
    ...
    def test_application_exception_auth_error_without_reauthentication_required_stays_generic(self):
        ...
        driver_exc = ProtoDriverException(
            ...
            status_code=STATUS_CODE_AUTHENTICATION_ERROR,
            error=ProtoDriverError(auth_error=ProtoAuthenticationError(detail="handshake failed")),
        )
        proto_exc = ProtoApplicationException(driver_exc)
    ...
    def test_application_exception_login_error_without_reauth_stays_plain_login_error(self):
        ...
        driver_exc = ProtoDriverException(
            ...
            error=ProtoDriverError(
                login_error=ProtoLoginError(message="Incorrect username or password", code=390100),
            ),
        )
        proto_exc = ProtoApplicationException(driver_exc)
