"""Python-specific E2E tests for session logout behavior.

These tests cover Python wrapper config (auto-detection, keep-alive remapping,
defaults, retry parameter) and atexit auto-cleanup. They are language-specific
and intentionally live in a separate file from test_logout.py (which covers
shared/session/logout.feature scenarios) so the orphan validator doesn't
require matching shared scenarios for these Python-only tests.
"""
...
class TestLogoutPythonWrapper:
    ...
    def test_should_skip_logout_when_server_session_keep_alive_is_true_regardless_of_auto_detection(
        self, int_test_connection_factory, core_proxy, wiremock, auto_detection: bool
    ) -> None:
    ...
    def test_should_use_best_effort_error_handling_strategy_by_default(
        self, int_test_connection_factory, core_proxy, caplog, wiremock
    ):
...
class TestAutoCleanup:
    ...
    def test_should_call_close_with_retry_false_from_atexit_handler(self, int_test_connection_factory, wiremock):
        ...
        # Reset the shared session-scoped Wiremock to clear Phase A mappings + journal,
        # then install Phase B mappings on the same JVM (formerly a second WiremockClient).
        wiremock.reset()
        wiremock.add_mapping("auth/login_success_jwt.json")
        wiremock.add_mapping("session/logout_500_always.json")
