...
PANDAS_TYPE_CASES = [
    # (type_name, value_expr, null_expr, value_check, dtype_check)
    ("number", "1::NUMBER", "NULL::NUMBER", lambda v: v == 1, pd.api.types.is_integer_dtype),
    (
        "scaled_number",
        "3.14::NUMBER(10,2)",
        "NULL::NUMBER(10,2)",
...
class TestFetchPandasAll:
    ...
    @pytest.mark.parametrize(
        "type_name,value_expr,null_expr,check,_dtype_check",
        PANDAS_TYPE_CASES,
        ids=[c[0] for c in PANDAS_TYPE_CASES],
    )
    def test_should_fetch_type_name_with_null_as_pandas_dataframe(
        self, execute_query, cursor, type_name, value_expr, null_expr, check, _dtype_check
    ):
    ...
    @pytest.mark.parametrize(
        "type_name,value_expr,_null_expr,_check,dtype_check",
        PANDAS_TYPE_CASES,
        ids=[c[0] for c in PANDAS_TYPE_CASES],
    )
    def test_should_return_empty_type_name_column_with_correct_pandas_dtype(
        self, execute_query, cursor, type_name, value_expr, _null_expr, _check, dtype_check
    ):
        # Given Snowflake client is logged in
        assert_connection_is_open(execute_query)
        ...
        assert dtype_check(result["COL"].dtype)
