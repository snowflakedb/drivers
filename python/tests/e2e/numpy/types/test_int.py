"""INT type tests for Universal Driver -- numpy consumer.

SQL INT/INTEGER/BIGINT/SMALLINT/TINYINT/BYTEINT are NUMBER(38,0) aliases.
``numpy=True`` does not choose a converter from that SQL name.
``FIXED_to_numpy_int64`` runs only when Arrow's physical type is a native
integer (INT8/16/32/64, scale 0). Decimal128 still uses
``DecimalFromDecimalConverter`` and returns Python ``int``.

Today the server narrows a column to a native Arrow integer when every
value in it fits in int64, and otherwise sends decimal128. The same ``INT``
column is therefore ``numpy.int64`` for typical magnitudes and Python ``int``
for 38-digit values. NULL is ``None``.

This file is the INT Gherkin family (public SQL types), not a claim that INT
is a non-numpy path. Declared ``NUMBER(p, 0)`` with ``p <= 18`` is covered in
``test_number.py``.
"""
...
class TestFetchNumpyIntTypeCasting:
    """INT synonyms with in-range values: Arrow native integer -> ``numpy.int64``.

    The converter is selected because the server sent a native integer array, not
    because the SQL type was named INT. 38-digit cases below take decimal128.
    """
    ...
    def test_should_cast_integer_values_to_appropriate_type_for_int_and_synonyms(self, cursor, int_type):
        ...
        assert_dtypes(df, [is_numpy_integer, is_numpy_integer, is_numpy_integer])
        assert get_row(df, 0) == [0, 1000000, INT64_MAX]


class TestFetchNumpyIntLiteral:
    """SELECT-with-literal coverage via numpy fetch.

    In-range literals follow the ``numpy.int64`` path. 38-digit literals force
    decimal128, so ``is_object`` (Python ``int``) is required — ``is_integer``
    would also accept ``numpy.int64`` and hide a converter-path change.
    """
    ...
    def test_should_handle_null_values_for_int_and_synonyms(self, cursor, int_type):
        ...
        assert get_row(df, 0) == [None, 42, None]
...
class TestFetchNumpyIntTable:
    """Table-based scenarios via numpy fetch.

    Mixed in-range rows stay on the native-integer path. The 38-digit table test
    asserts ``is_object`` because those values force decimal128 / Python ``int``.
    """
    ...
    def test_should_select_values_from_table_for_int_and_synonyms(
        ...
        table_name = f"{tmp_schema}.np_int_table_{int_type.lower()}_{values}"
        ...
        col = get_column(df, 0)
        if has_null:
            assert col == [None if v is None else v for v in expected_values]
        else:
    ...
    def test_should_select_large_integer_values_from_table_for_int_and_synonyms(
        ...
        # [-99999999999999999999999999999999999999, 99999999999999999999999999999999999999]
        table_name = f"{tmp_schema}.np_int38_table_{int_type.lower()}"
        execute_query(f"CREATE OR REPLACE TEMPORARY TABLE {table_name} (col {int_type})")
    ...
    def test_should_handle_server_side_arrow_memory_optimization_for_int_columns_on_multiple_chunks(
        ...
        table_name = f"{tmp_schema}.np_different_int_column_sizes"
        execute_query(
...
class TestFetchNumpyIntBinding:
    ...
    def test_should_insert_integer_using_parameter_binding_for_int_and_synonyms(
        ...
        table_name = f"{tmp_schema}.np_int_bind_{int_type.lower()}"
        execute_query(f"CREATE OR REPLACE TEMPORARY TABLE {table_name} (col {int_type})")
    ...
    def test_should_insert_and_select_integers_from_table_using_batch_parameter_binding_for_int_and_synonyms(
        ...
        # And Table with <type> column exists
        table_name = f"{tmp_schema}.np_int_batch_bind_{int_type.lower()}"
        execute_query(f"CREATE OR REPLACE TEMPORARY TABLE {table_name} (col {int_type})")
