"""TIMESTAMP_NTZ type tests for Universal Driver -- numpy consumer.

Mirrors ``tests/definitions/shared/types/timestamp_ntz.feature`` and
``python/tests/e2e/types/test_timestamp_ntz.py`` using
``numpy=True`` cursor fetch (``fetchone`` / ``fetchall``).

Arrow timestamp -> naive ``numpy.datetime64[ns]`` cells. NULL maps to ``None``.
"""
...
def sequential_timestamp(i):
    return to_datetime64_ns(SEQUENTIAL_TIMESTAMP_BASE + timedelta(seconds=i))


def ns_values(values):
    return [None if v is None else to_datetime64_ns(v) for v in values]


class TestFetchNumpyTimestampNtzTypeCasting:
    """Type-casting coverage for TIMESTAMP_NTZ via numpy fetch."""
    ...
    def test_should_cast_timestamp_ntz_values_to_appropriate_type(self, cursor):
        ...
        assert val == to_datetime64_ns(TS_2024_JAN)
        # And Values should not have timezone info
        assert_timezone((val,), expected_tz=None)


class TestFetchNumpyTimestampNtzLiteral:
    ...
    def test_should_select_timestamp_ntz_values(self, cursor, query_values, expected_values):
        ...
        assert tuple(row) == tuple(ns_values(expected_values))
        # And Values should not have timezone info
    ...
    def test_should_handle_null_values_for_timestamp_ntz(self, cursor):
        ...
        assert row[0] == to_datetime64_ns(TS_2024_JAN)
...
class TestFetchNumpyTimestampNtzTable:
    ...
    def test_should_select_values_from_table_for_timestamp_ntz(
        ...
        table_name = f"{tmp_schema}.np_ts_ntz_table_{values_name}"
        ...
        col = get_column(df, 0)
        assert_dtypes(df, [is_datetime64])
        assert col == ns_values(expected_values)
        # And Values should not have timezone info
    ...
    def test_should_download_large_result_set_with_multiple_chunks_from_table_for_timestamp_ntz(
        ...
        table_name = f"{tmp_schema}.np_large_ts_ntz_table"
        execute_query(f"CREATE OR REPLACE TEMPORARY TABLE {table_name} (col TIMESTAMP_NTZ)")
        execute_query(
...
class TestFetchNumpyTimestampNtzBinding:
    ...
    def test_should_select_timestamp_ntz_using_parameter_binding(self, cursor):
        ...
        row = get_row(df, 0)
        assert_dtypes(df, [is_datetime64, is_datetime64])
        assert tuple(row) == (to_datetime64_ns(TS_2024_JAN), to_datetime64_ns(TS_2024_JUN))
        # And Values should not have timezone info
        assert_datetime_type(row)
        assert_timezone(row, expected_tz=None)
    ...
    def test_should_insert_timestamp_ntz_using_parameter_binding(
        ...
        table_name = f"{tmp_schema}.np_ts_ntz_bind"
        execute_query(f"CREATE OR REPLACE TEMPORARY TABLE {table_name} (col TIMESTAMP_NTZ)")
        ...
        col = get_column(df, 0)
        assert_dtypes(df, [is_datetime64])
        assert col[0] == to_datetime64_ns(TS_2024_JAN)
        assert col[1] == to_datetime64_ns(TS_2024_JUN)
    ...
    def test_should_store_utc_equivalent_when_binding_timezone_aware_datetime_to_timestamp_ntz(
        ...
        assert_dtypes(df, [is_datetime64])
        val = get_row(df, 0)[0]
        assert val == to_datetime64_ns(expected)
        # And Values should not have timezone info
        assert_timezone((val,), expected_tz=None)


class TestFetchNumpyTimestampNtzAliases:
...
class TestFetchNumpyTimestampNtzPrecision:
    """``numpy.datetime64`` retains nanoseconds from Arrow (vs Python ``datetime`` μs cap).
    ...
    values from ``cursor.fetchall()`` with ``numpy=True``.
    """
    ...
    def test_should_truncate_nanosecond_precision_to_microseconds_for_timestamp_ntz(
        ...
        assert_dtypes(df, [is_datetime64])
        val = get_row(df, 0)[0]
        assert val == np.datetime64(input_str.replace(" ", "T"))
        ns_frac = int(val.astype("datetime64[ns]").astype(np.int64) % 1_000_000_000)
        assert ns_frac == expected_microsecond * 1000 + expected_nanosecond
        # And Values should not have timezone info
        assert_datetime_type((val,))
        assert_timezone((val,), expected_tz=None)


class TestFetchNumpyTimestampNtzNegativeEpoch:
    """Sub-second TIMESTAMP_NTZ instants just before the Unix epoch, via numpy fetch.
    ...
    ``numpy.datetime64`` retains the full nanosecond fraction. The two scales cover both NTZ
    ...
    def test_should_select_sub_second_timestamp_ntz_values_before_epoch(self, cursor, query, expected):
        ...
        assert_dtypes(df, [is_datetime64])
        val = get_row(df, 0)[0]
        assert val == np.datetime64(expected.replace(" ", "T"))
        assert_timezone((val,), expected_tz=None)
