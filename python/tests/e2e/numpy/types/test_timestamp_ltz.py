"""TIMESTAMP_LTZ type tests for Universal Driver -- numpy consumer.
...
class TestFetchNumpyTimestampLtzTypeCasting:
    """Type-casting coverage for TIMESTAMP_LTZ via numpy fetch."""
...
class TestFetchNumpyTimestampLtzLiteral:
...
class TestFetchNumpyTimestampLtzTable:
    ...
    def test_should_select_values_from_table_for_timestamp_ltz(
        ...
        table_name = f"{tmp_schema}.np_timestamp_ltz_table_{values_name}"
    ...
    def test_should_download_large_result_set_with_multiple_chunks_from_table_for_timestamp_ltz(
        ...
        table_name = f"{tmp_schema}.np_large_timestamp_ltz_table"
        execute_query(f"CREATE OR REPLACE TEMPORARY TABLE {table_name} (col TIMESTAMP_LTZ)")
        execute_query(
...
class TestFetchNumpyTimestampLtzBinding:
    ...
    def test_should_insert_timestamp_ltz_using_parameter_binding(
        ...
        table_name = f"{tmp_schema}.np_timestamp_ltz_bind"
        execute_query(f"CREATE OR REPLACE TEMPORARY TABLE {table_name} (col TIMESTAMP_LTZ)")
...
class TestFetchNumpyTimestampLtzNegativeEpoch:
    """Sub-second TIMESTAMP_LTZ instants just before the Unix epoch, via numpy fetch.
