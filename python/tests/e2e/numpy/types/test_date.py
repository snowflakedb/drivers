"""DATE type tests for Universal Driver -- numpy consumer.
...
DATE_2024_JAN = to_datetime64_d(date(2024, 1, 15))
DATE_1999_DEC = to_datetime64_d(date(1999, 12, 31))
DATE_EPOCH = to_datetime64_d(date(1970, 1, 1))
DATE_PRE_EPOCH = to_datetime64_d(date(1969, 12, 31))
DATE_1900 = to_datetime64_d(date(1900, 1, 1))
DATE_HISTORICAL_MIN = to_datetime64_d(date(1, 1, 1))
DATE_100_MAR = to_datetime64_d(date(100, 3, 1))
DATE_GREGORIAN = to_datetime64_d(date(1582, 10, 15))
DATE_MAX = to_datetime64_d(date(9999, 12, 31))
...
def sequential_date(i):
    ...
    return to_datetime64_d(SEQUENTIAL_BASE + timedelta(days=i))


class TestFetchNumpyDateTypeCasting:
    """Type-casting coverage for DATE via numpy fetch."""
...
class TestFetchNumpyDateTable:
    ...
    def test_should_select_dates_from_table(self, execute_query, cursor, tmp_schema):
        ...
        table_name = f"{tmp_schema}.np_date_table"
    ...
    def test_should_select_dates_with_null_from_table(self, execute_query, cursor, tmp_schema):
        ...
        table_name = f"{tmp_schema}.np_date_null_table"
        execute_query(f"CREATE OR REPLACE TEMPORARY TABLE {table_name} (col DATE)")
    ...
    def test_should_select_historical_and_boundary_dates_from_table(self, execute_query, cursor, tmp_schema):
        ...
        table_name = f"{tmp_schema}.np_date_historical_table"
        execute_query(f"CREATE OR REPLACE TEMPORARY TABLE {table_name} (col DATE)")
        execute_query(
    ...
    def test_should_download_large_result_set_for_date_from_table(self, execute_query, cursor, tmp_schema):
        ...
        table_name = f"{tmp_schema}.np_date_large_table"
        execute_query(f"CREATE OR REPLACE TEMPORARY TABLE {table_name} (col DATE)")
        execute_query(
...
class TestFetchNumpyDateBinding:
    ...
    def test_should_select_date_using_parameter_binding(self, cursor):
        ...
        df = execute_and_fetch(
            ...
            params=(DATE_2024_JAN.item(), DATE_EPOCH.item(), DATE_1999_DEC.item()),
        )
    ...
    def test_should_insert_date_using_parameter_binding(self, execute_query, executemany_insert, cursor, tmp_schema):
        ...
        table_name = f"{tmp_schema}.np_date_bind"
        execute_query(f"CREATE OR REPLACE TEMPORARY TABLE {table_name} (col DATE)")

        # When Date values [2024-01-15, 1970-01-01, 1999-12-31] are inserted using parameter binding
        test_data = [(DATE_2024_JAN.item(),), (DATE_EPOCH.item(),), (DATE_1999_DEC.item(),)]
