"""Semi-structured type (VARIANT/OBJECT/ARRAY) tests for Universal Driver -- numpy consumer.
...
class TestFetchNumpySemiStructuredTypeCasting:
    """Type-casting coverage for VARIANT/OBJECT/ARRAY via numpy fetch."""
...
class TestFetchNumpySemiStructuredLiteral:
...
class TestFetchNumpySemiStructuredNullLiteral:
    """NULL handling for semi-structured literals via numpy fetch."""
...
class TestFetchNumpySemiStructuredTable:
    ...
    def test_should_select_semi_structured_values_from_table(self, execute_query, cursor, tmp_schema):
        ...
        table_name = f"{tmp_schema}.np_semi_table"
        execute_query(
    ...
    def test_should_handle_null_semi_structured_values_from_table(self, execute_query, cursor, tmp_schema):
        ...
        table_name = f"{tmp_schema}.np_semi_null"
        ...
        # When Query "SELECT * FROM <table>" is executed
        df = execute_and_fetch(cursor, f"SELECT col FROM {table_name} ORDER BY id")

        # Then Result should contain [NULL, {"a":1}, NULL]
        assert_dtypes(df, [is_string])
        assert get_row(df, 0)[0] is None
        assert json.loads(get_row(df, 1)[0]) == {"a": 1}
        assert get_row(df, 2)[0] is None


class TestFetchNumpySemiStructuredEmptyContainers:
    """Empty JSON container scenarios via numpy fetch."""
    ...
    def test_should_round_trip_empty_json_containers_through_a_table(self, execute_query, cursor, tmp_schema):
        ...
        table_name = f"{tmp_schema}.np_semi_empty"
        execute_query(
            f"CREATE OR REPLACE TEMPORARY TABLE {table_name} (col_variant VARIANT, col_object OBJECT, col_array ARRAY)"
        )
...
class TestFetchNumpySemiStructuredUnicode:
    """Unicode content scenarios via numpy fetch."""
...
class TestFetchNumpySemiStructuredMultipleChunks:
    """Multiple-chunk download scenarios via fetchall (all chunks)."""
...
class TestFetchNumpySemiStructuredBinding:
    ...
    def test_should_insert_variant_using_parameter_binding(self, execute_query, cursor, tmp_schema):
        ...
        table_name = f"{tmp_schema}.np_semi_bind"
