...
class TestBasicTypeBinding:
    ...
    def test_should_bind_integer_types(self, cursor):
        ...
        for int_val in test_cases:
            cursor.execute("SELECT ?", (int_val,))
            result = cursor.fetchone()
            assert result == (int_val,), f"Failed for value {int_val}"

    def test_should_bind_float_types(self, cursor):
        ...
        test_cases = [
            ...
            3.14159,
            -3.14159,
            1.7e308,  # Large positive
            -1.7e308,  # Large negative
            2.2e-308,  # Small positive
            -2.2e-308,  # Small negative
        ]

        for float_val in test_cases:
            cursor.execute("SELECT ?", (float_val,))
            result = cursor.fetchone()
            assert result is not None
            # Use relative tolerance for float comparison
            if abs(float_val) > 1e10:
                assert abs(result[0] - float_val) <= abs(float_val) * 1e-14
            else:
                assert abs(result[0] - float_val) < 1e-10

    def test_should_bind_string_types(self, cursor):
        ...
        test_cases = [
            ...
            "Mixed 'quotes' and \"quotes\"",  # Mixed quotes
            "Backslash \\",  # Backslash
        ]

        for str_val in test_cases:
            cursor.execute("SELECT ?::VARCHAR", (str_val,))
            result = cursor.fetchone()
            assert result == (str_val,), f"Failed for value {str_val!r}"

    def test_should_bind_boolean_types(self, cursor):
        # Test boolean values
        cursor.execute("SELECT ?, ?", (True, False))
        result = cursor.fetchone()
        assert result == (True, False)

        # Test boolean with explicit casting
        cursor.execute("SELECT ?::BOOLEAN, ?::BOOLEAN", (True, False))
        result = cursor.fetchone()
        assert result == (True, False)
