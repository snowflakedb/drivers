...
class TestTimestampTzNegativeEpochJsonResultFormat:
    """Sub-second TIMESTAMP_TZ instants before the Unix epoch, over JSON result format.
    ...
    scales cover both TZ builders: scale 9 (struct epoch+fraction+tz) and scale 3
    (combined Int64 epoch + tz).
    """
    ...
    def test_should_select_sub_second_timestamp_tz_values_before_epoch(self, connection_factory, query, expected):
        # Given Snowflake client is logged in
