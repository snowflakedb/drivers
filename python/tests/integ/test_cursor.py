...
class TestCursorDescribeInternal:
    ...
    def test_describes_parameterized_call_to_table_procedure(self, cursor):
        ...
        itself, unmodified, together with the real bind values. The procedure name is
        itself a bind (`IDENTIFIER(?)`) so describe cannot resolve RETURNS TABLE from
        the SQL text alone.
        """
        proc_name = f"test_di_call_{uuid.uuid4().hex[:8]}"
        cursor.execute(
            f"""
        ...
        result = cursor._describe_internal("CALL IDENTIFIER(?)(?, ?)", (proc_name, 2, 3))
