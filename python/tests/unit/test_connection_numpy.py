...
class TestConnectionNumpyParameter:
    ...
    def test_numpy_true_sets_attribute(self, mock_db_api):
        with (
            patch("snowflake.connector.connection.check_dependency"),
            patch("snowflake.connector.aio.connection.check_dependency"),
        ):
    ...
    def test_numpy_true_without_numpy_installed_raises(self, mock_db_api):
        ...
        with (
            patch("snowflake.connector.connection.np", missing),
            patch("snowflake.connector.aio.connection.np", missing),
        ):
