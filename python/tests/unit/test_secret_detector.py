...
class TestMaskSecrets:
    ...
    def test_password_is_masked(self):
        ...
        masked, masked_text, err_str = SecretDetector.mask_secrets("password:" + random_password)
        assert masked
        assert err_str is None
        ...
        masked, masked_text, err_str = SecretDetector.mask_secrets("pwd:" + random_password)
        assert masked
        assert err_str is None
    ...
    def test_connection_token_is_masked(self):
        ...
        masked, masked_text, err_str = SecretDetector.mask_secrets("Token =" + long_token)
        assert masked
        assert err_str is None
        assert masked_text == "Token =****"

        masked, masked_text, err_str = SecretDetector.mask_secrets("idToken : " + long_token)
        assert masked
        assert err_str is None
        assert masked_text == "idToken : ****"

        masked, masked_text, err_str = SecretDetector.mask_secrets("sessionToken : " + long_token)
        assert masked
        assert err_str is None
        assert masked_text == "sessionToken : ****"

        masked, masked_text, err_str = SecretDetector.mask_secrets("masterToken : " + long_token)
        assert masked
        assert err_str is None
        assert masked_text == "masterToken : ****"

        masked, masked_text, err_str = SecretDetector.mask_secrets("assertion content:" + long_token)
        assert masked
        assert err_str is None
    ...
    def test_multiple_secrets_are_all_masked(self):
        ...
        assert err_str is None
        assert (
            masked_text == "token=****" + " random giberish " + "password:****" + " random giberish " + "idToken:****"
        )
        ...
        assert (
            masked_text
            == "password=****" + " random giberish " + "password=****" + " random giberish " + "password=****"
        )
    ...
    def test_private_key_body_is_masked(self):
        ...
        assert masked
        assert err_str is None
        assert masked_text == "-----BEGIN PRIVATE KEY-----\\nXXXX\\n-----END PRIVATE KEY-----\n"
