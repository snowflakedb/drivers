...
class TestAsyncSendLogBatch:
    def test_forwards_to_core(self):
        ...
        with patch.object(async_core_driver, "telemetry_send_log_batch", new_callable=AsyncMock) as rpc:
            asyncio.run(client.send_log_batch())
        rpc.assert_awaited_once_with(conn_handle=_CONN_HANDLE)

    def test_disables_client_on_rpc_failure(self):
        client = _client()
        with patch.object(
            async_core_driver, "telemetry_send_log_batch", new_callable=AsyncMock, side_effect=RuntimeError("boom")
        ):
            asyncio.run(client.send_log_batch())  # must not raise
...
class TestAsyncClose:
    ...
    def test_log_batch_is_alias_for_send_log_batch(self):
        assert AsyncTelemetryClient._log_batch is AsyncTelemetryClient.send_log_batch
