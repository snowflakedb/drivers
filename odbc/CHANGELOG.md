...
- Fixed `SQLTables` `REMARKS` to return `SQL_NULL_DATA` when no table comment is available (was an empty string), and to surface a real `SHOW OBJECTS` comment when present; absent `SQLColumns` / `SQLProcedures` `REMARKS` and `SQLColumns` `COLUMN_DEF` likewise return `SQL_NULL_DATA` rather than `""`. (snowflake-eng/drivers#1148)
