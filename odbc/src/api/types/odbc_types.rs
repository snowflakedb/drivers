...
pub struct Connection {
    ...
    /// `Weak::as_ptr`).
    pub child_statements: Vec<(Weak<Statement>, *const Statement)>,
...
// Safety: Connection is always accessed from the single ODBC thread that holds the handle.
// `*const Statement` inside `child_statements` is `!Send`, but Connection is never
// moved across threads — it lives in a `Box` whose raw pointer is the opaque DBC handle.
...
impl Statement {
    ...
    pub fn conn_ptr(&self) -> *mut Connection {
        self.conn
    }
}
