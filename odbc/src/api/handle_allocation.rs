...
pub fn free_statement(handle: sql::Handle) -> OdbcResult<()> {
    ...
    // Remove the corresponding Weak entry from the connection's bookkeeping list.
    ...
    let stmt_handle = stmt.stmt_handle;
    // child_statements entry removed above; drop frees the Arc data.
    drop(stmt);
