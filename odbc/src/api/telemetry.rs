...
//! tokio runtime that issues the RPC. The SQL hot path is not blocked, and
//! protobuf encode/decode happens off the caller's thread. Calls for
...
///
/// Best-effort: returns immediately without blocking the caller. If the
/// runtime is not initialised, the handle does not resolve to a connected
/// session, or the RPC fails for any reason, the event is silently dropped.
pub fn record_api_usage(
    ...
    rt.spawn_telemetry(move |client| async move {
        let _ = client
...
/// `OdbcError` itself is **not** moved into the spawned task — only the
/// already-classified `(exception_type, error_source)` strings (returned by
/// [`OdbcError::telemetry_classification`]) are, so the borrow on the
/// caller's `&OdbcError` does not outlive this function.
