...
mod tests {
    ...
    // ======================================================================
    // write_wire: non-finite f64 serialization for Snowflake's bind parser
    //
    // The server's bind parser is stricter than its TO_DOUBLE() text
    ...
    fn write_wire(value: f64) -> String {
        SnowflakeReal.write_wire(value).unwrap()
    }

    #[test]
    fn write_wire_finite_values_use_default_display() {
        assert_eq!(write_wire(0.0), "0");
        assert_eq!(write_wire(1.5), "1.5");
        assert_eq!(write_wire(-12345.6789), "-12345.6789");
    }

    #[test]
    fn write_wire_nan_serializes_as_capital_n_a_n() {
        assert_eq!(write_wire(f64::NAN), "NaN");
    }

    #[test]
    fn write_wire_positive_infinity_serializes_as_full_word_infinity() {
        // Server rejects Rust's default "inf" — must be the full word.
        assert_eq!(write_wire(f64::INFINITY), "Infinity");
    }

    #[test]
    fn write_wire_negative_infinity_serializes_as_full_word_with_sign() {
        assert_eq!(write_wire(f64::NEG_INFINITY), "-Infinity");
    }
}
