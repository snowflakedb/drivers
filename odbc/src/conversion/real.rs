...
/// Returns `true` when `s` (already trimmed) is a token Rust's `f64::from_str`
/// maps to a non-finite value without overflowing a decimal literal:
/// `nan`, `inf`, `infinity`, optional leading `+`/`-`, ASCII-case-insensitive.
...
/// Accepts a finite parse and explicit non-finite tokens (`Infinity`, `NaN`,
/// `inf`, any ASCII case or leading sign). Overflowing decimal literals that
/// Rust silently maps to `+/-inf` (e.g. `"1e309"`) error as
/// `NumericMagnitudeOverflow` (SQLSTATE 22003).
fn reject_overflowed_real_literal(trimmed: &str, v: f64) -> Result<(), BindingError> {
    if v.is_finite() || is_explicit_non_finite_token(trimmed) {
        return Ok(());
    }
