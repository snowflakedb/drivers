...
use crate::conversion::error::{
    IntervalFieldOverflowSnafu, InvalidValueSnafu, NumericValueOutOfRangeSnafu, WriteOdbcError,
};
...
/// Parse `<seconds>[.<fraction>]` into `(second, fraction_micros, has_fraction)`.
/// The fraction is normalised to microseconds (always 6 digits internally),
/// truncating extra digits and zero-padding shorter fractions.
fn parse_seconds_with_fraction(s: &str) -> Result<(u128, u32, bool), WriteOdbcError> {
    if let Some((whole, frac)) = s.split_once('.') {
        ...
        Ok((second, micros, true))
    } else {
        Ok((parse_u128(s)?, 0, false))
    }
}
    ...
    if let Some(time_str) = time_part {
        ...
        match parts.len() {
            1 => {
                ...
                // hour/minute/second).
                let (sec, micros, has_frac) = parse_seconds_with_fraction(parts[0].trim())?;
                if day_part.is_some() {
                    ...
                    p.minute = Some(sec);
                    p.second = Some(sec);
                    p.fraction_micros = Some(micros);
                    p.has_fraction = has_frac;
            ...
            2 => {
                ...
                // (fraction sticks to seconds only). If the trailing
                // component carries a `.`, the input can ONLY be
                // M:S form, so we store it that way; otherwise we
                // store the H:M reading and let the
                // `IntervalMinuteToSecond` arm in `build_composite`
                // re-interpret as M:S.
                let (sec, micros, has_frac) = parse_seconds_with_fraction(parts[1].trim())?;
                if has_frac {
                    p.minute = Some(parse_u128(parts[0].trim())?);
                    p.second = Some(sec);
                    p.fraction_micros = Some(micros);
                    p.has_fraction = true;
                } else {
            ...
            3 => {
                ...
                let (sec, micros, has_frac) = parse_seconds_with_fraction(parts[2].trim())?;
                p.second = Some(sec);
                p.fraction_micros = Some(micros);
                p.has_fraction = has_frac;
            }
...
fn build_single_field(
    ...
    // `numeric_helpers::write_single_field_interval`.
    let is_negative = parts.is_negative && field > 0;
...
/// non-zero field that the target qualifier cannot represent.
...
///   * `NumericValueOutOfRange` → 22003 (numeric out of range)
pub(crate) fn varchar_to_interval(
    ...
    match target {
        ...
        _ => NumericValueOutOfRangeSnafu {
            reason: format!("varchar_to_interval called with non-interval target {target:?}"),
        }
        .fail(),
    }
}
