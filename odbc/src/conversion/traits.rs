...
use crate::api::encoding::{
    WCharEncoding, WideChar, current_wchar_encoding, wchar_byte_size, wide_unit_len,
...
impl Binding {
    ...
    /// Write wide-char data to a wide-char buffer, with each DM-side code
    /// unit generated by a function. `total_char_count` is the total number
    /// of output wide characters (DM-side code units). The indicator is set
    /// in bytes (chars * [`wchar_byte_size`]) per ODBC wide-char convention;
    /// the runtime DM-side `SQLWCHAR` width is 2 bytes for UTF-16 and 4
    /// bytes for UTF-32.
        ...
        F: Fn(usize) -> Option<WideChar>,
    {
        ...
        let max_write_len = ((self.buffer_length / unit_size) - 1) as usize;
        let target = self.target_value_ptr as *mut WideChar;
        // The closure produces a `u16` per DM-side unit. ASCII fits in
        // either encoding via a zero-extending cast; supplementary-plane
        // code points are not supported by the current callers (hex
        // digits) and would be silently truncated to the BMP via the
        // `as u32` cast — that's fine because all current converters
        // return ASCII bytes.
        let written = match current_wchar_encoding() {
        ...
        unsafe { write_wide_null(target, written) };
    ...
    pub fn write_wchar_string(&self, src: &str, get_data_offset: &mut Option<usize>) -> Warnings {
        ...
        let offset = get_data_offset.unwrap_or(0);
        let max_units = (self.buffer_length / unit_size) as usize;
        let value_ptr = self.target_value_ptr as *mut WideChar;
        let writable = max_units.saturating_sub(1);
        let written = unsafe { write_wide_buffer(src, value_ptr, writable, offset) };
        unsafe { write_wide_null(value_ptr, written) };

        let new_offset = offset + written;
        if new_offset < total_units {
            // Return remaining byte count instead of SQL_NO_TOTAL (BD#23).
            // The ODBC spec says the indicator should contain the data
            // length when determinable, and ours always is.
            let remaining_bytes = ((total_units - offset) as sql::Len) * unit_size;
            let _ = self.write_length_or_null(LengthOrNull::Length(remaining_bytes));
            *get_data_offset = Some(new_offset);
            vec![Warning::StringDataTruncated]
        } else {
            // COMPATIBILITY: ODBC 3.80 specification says that the string
            // length should be the number of characters, not the number of
            // bytes. However, older versions of Snowflake ODBC driver
            // returns the number of bytes.
            let num_bytes = (written as sql::Len) * unit_size;
