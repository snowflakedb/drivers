...
impl Binding {
        ...
        F: Fn(usize) -> Option<WideChar>,
    {
        ...
        // The converter produces a `u16` per DM-side unit. All current
        // callers (hex digits, fixed ASCII glyphs) yield ASCII bytes, so
        // widening to UTF-32 via `as u32` is a zero-extending cast.
    ...
    pub fn write_wchar_string(&self, src: &str, get_data_offset: &mut Option<usize>) -> Warnings {
        ...
        if new_offset < total_units {
            ...
            *get_data_offset = Some(new_offset);
            ...
            // COMPATIBILITY: ODBC 3.80 says the indicator should hold the
            // character count, but older Snowflake ODBC drivers always
            // wrote the byte count, so we preserve the byte-count form.
