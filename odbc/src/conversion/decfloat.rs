...
pub(crate) fn i128_from_big_endian_signed(bytes: &[u8]) -> i128 {
    if bytes.is_empty() {
        return 0;
    }
...
impl ReadArrowType<StructArray> for SnowflakeDecfloat {
    fn read_arrow_type<'a>(
        ...
        let exponent_array = array
            ...
            .expect("DECFLOAT struct must have 'exponent' field")
            .as_any()
            .downcast_ref::<arrow::array::PrimitiveArray<Int16Type>>()
            .expect("exponent field should be Int16");

        let significand_array = array
            .column_by_name("significand")
            .expect("DECFLOAT struct must have 'significand' field")
            .as_any()
            .downcast_ref::<GenericByteArray<GenericBinaryType<i32>>>()
            .expect("significand field should be Binary");
