...
sequenceDiagram
    ...
    participant LB as Loopback listener (127.0.0.1:port)
    participant Browser
    participant IdP as Authorization Server
    participant SF as Snowflake /session/v1/login-request
    ...
    alt Cached access token
        ...
        Driver->>LB: Bind 127.0.0.1:random-port
        Driver->>Browser: open(authorize_url?response_type=code&client_id=...&code_challenge=...&state=...)
        Browser->>IdP: GET /authorize
        IdP-->>Browser: 302 redirect to loopback?code=...&state=...
        Browser->>LB: GET / with code + state
        LB-->>Browser: HTML "Authorization completed."
        LB->>Driver: Validate state, extract code
        Driver->>IdP: POST token endpoint (grant_type=authorization_code, code, code_verifier, redirect_uri)
...
sequenceDiagram
    ...
    App->>Driver: connect(authenticator=OAUTH_CLIENT_CREDENTIALS, oauth_client_id, oauth_client_secret, oauth_token_request_url, ...)
    Driver->>IdP: POST token endpoint (grant_type=client_credentials, scope, Basic auth)
    IdP-->>Driver: {access_token, expires_in, ...}
    Driver->>SF: AUTHENTICATOR=OAUTH + TOKEN=[access_token]
    Note right of Driver: CLIENT_ENVIRONMENT.OAUTH_TYPE=oauth_client_credentials
    SF-->>Driver: Session token
    Driver-->>App: Connection ready
```
