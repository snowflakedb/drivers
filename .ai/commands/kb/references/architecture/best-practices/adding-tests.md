---
description: Where a new Universal Driver test belongs — choosing the directory and tier (unit, integ, e2e) per wrapper, and when a Gherkin feature scenario is required alongside it.
no-pointer: true
---
...
- `jdbc/src/test/java/net/snowflake/jdbc/integration/` = WireMock-based; stubs the server, no live Snowflake needed.
- `jdbc/src/test/java/net/snowflake/jdbc/e2e/` = live Snowflake account required.
...
2. Place the test in the matching directory (same filename stem as the feature):
   - JDBC e2e: `jdbc/src/test/java/net/snowflake/jdbc/e2e/<category>/<FeatureName>Tests.java`
...
└── No → driver-specific, no feature needed
    ...
    ├── WireMock only? (JDBC) → jdbc/src/test/.../jdbc/integration/
...
| Live-server test placed in `net/snowflake/jdbc/integration/` | Wrong tree — `integration/` is WireMock-only | Move to `net/snowflake/jdbc/e2e/` |
