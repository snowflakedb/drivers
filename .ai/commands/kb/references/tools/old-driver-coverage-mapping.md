---
description: The tests/oldTestsCoverage/{jdbc,odbc,python}.yaml coverage files — their schema, how an old test name maps to its Universal Driver equivalent, unmapped-test and coverage-gap queries, reverse lookup, and reconciling the summary counts. Not code-coverage metrics.
no-pointer: true
---
...
see [../architecture/repos/legacy-drivers.md](../architecture/repos/legacy-drivers.md).
...
tests:
  ...
  - test_name: should authenticate using external browser
    ud_tests:                          # flat list; odbc.yaml separator is " - " (many-to-many)
    - sf_core/tests/e2e/auth/oauth_test.rs - test_oauth_basic
    - sf_core/tests/unit/auth/token_exchange_test.rs - test_exchange_flow
  ...
  - test_name: should throw error for wrong okta credentials
    ud_tests:
    - sf_core/tests/e2e/authentication/native_okta.rs - vpn_should_fail
...
- **The schema above is the target, not what the files hold.** Every entry in all three files carries `test_name` and `ud_tests` and nothing else, except `notes` which appears only in `python.yaml`; `status`, `gaps`, and `jira` appear in none of them. Treat an empty `ud_tests` as `unmapped` and a non-empty one as `mapped`, and offer to migrate entries to the full schema on first edit.
- **The `ud_tests` separator is per file, not repo-wide:** ` - ` in `odbc.yaml`, `#` in `jdbc.yaml`, `::` in `python.yaml`, and it follows the file rather than the UD test's language. The example above is an `odbc.yaml` extract, so it uses ` - `. Match the file you are editing — the table is in [../concepts/old-driver-coverage-attribution.md](../concepts/old-driver-coverage-attribution.md).
...
When a gap involves a deliberate behavioral change between old and UD, check that wrapper's `BehaviorDifferences.yaml` before writing the gap. The four paths and the entry schema are in [../concepts/behavior-differences.md](../concepts/behavior-differences.md).
