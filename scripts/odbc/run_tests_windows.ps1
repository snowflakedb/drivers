...
#
# -SkipBuild  When set, skip cmake configure+build entirely and go straight to
#             ctest.  Use this when cmake-build/ was pre-populated by a
#             build-once CI job and only the ctest run is needed.
[CmdletBinding()]
param(
    [switch]$SkipBuild,
...
try {
    ...
    if (-not $SkipBuild) {
        ...
        Write-Host "run_tests: -SkipBuild set — using pre-built cmake-build/"
        # Detect generator from the pre-built tree so ctest gets correct -C flag.
        $cacheFile = "cmake-build\CMakeCache.txt"
        $existingGenerator = if (Test-Path $cacheFile) {
            (Select-String -Path $cacheFile -Pattern '^CMAKE_GENERATOR:INTERNAL=(.*)$' |
                ForEach-Object { $_.Matches[0].Groups[1].Value.Trim() } |
                Select-Object -First 1)
        }
        $useNinja = $existingGenerator -and $existingGenerator.StartsWith("Ninja")
        Write-Host "run_tests: detected generator '$existingGenerator'"
    }
