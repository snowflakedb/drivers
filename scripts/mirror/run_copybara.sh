...
#   DRIVER_MIRROR_TOKEN     Push token for snowflakedb/universal-driver
  ...
  import)
    ...
    docker run --rm \
      ...
      migrate ci/mirroring/copy.bara.sky import "${PR_NUMBER}" \
        --git-destination-url="https://x-access-token:${DRIVER_MIRROR_TOKEN}@github.com/snowflakedb/universal-driver.git" \
