...
use jni::JNIEnv;
use sf_core::c_api::{sf_core_api_destroy, sf_core_api_flush, sf_core_api_init, sf_core_api_read, sf_core_api_write, CApiHandle, SfCoreApi};
...
fn handle_to_jlong_array<'a>(env: &mut JNIEnv<'a>, handle: CApiHandle) -> JLongArray<'a> {
    ...
    env.set_long_array_region(&array, 0, &[handle.id as i64, handle.magic as i64]).unwrap();
    array
}
...
pub extern "system" fn Java_com_snowflake_jdbc_CoreTransport_nativeRead(
    ...
    if bytes_read > 0 {
        ...
        if let Err(_) = env.set_byte_array_region(
            &java_buffer,
            0,
            &read_buffer[..bytes_to_copy] as &[i8],
        ) {
            return -1;
        }
    }
